...
def make_dac_calibrator(k: int, device: torch.device, backend: str):
    ...
    if backend == "faiss":
        ...
        raise ValueError(
            f"Invalid DAC kNN backend: {backend}. Must be 'faiss' or 'torch'"
        )
...
def make_geometric_predict_fn(
    ...
    def predict_fn(Xc):
        ...
        calibrated_probs = geo_calibrator.calibrate(
            X_test_embed=test_features_np, X_test_original=Xc
        )
...
def make_sgc_predict_fn(
    ...
    def predict_fn(Xc):
        ...
        dummy_loader = DataLoader(
            TensorDataset(torch.from_numpy(Xc[:1]).float()), batch_size=1, shuffle=False
        )
        ...
        # Get calibrated probabilities
        calibrated_probs = geo_calibrator.calibrate(
            X_test_embed=test_features, X_test_original=Xc
        )
...
def make_coordinate_predict_fn(
    ...
    def predict_fn(Xc):
        ...
        # Get calibrated probabilities
        calibrated_probs = geo_calibrator.calibrate(
            X_test_embed=test_coords, X_test_original=Xc
        )
...
def evaluate_corruption_robustness(
    ...
    if (
        "standard_baselines" in results
        and "uncalibrated" in results["standard_baselines"]
    ):
    ...
    if (
        "standard_baselines" in results
        and "temperature_scaling" in results["standard_baselines"]
    ):
        ...
        try:
            ...
            if "temperature" in ts_data:
                ...
                predict_fn = make_temperature_scaling_predict_fn(
                    model_adapter, ts_cal, batch_size
                )
                mce = compute_mce_for_method(
                    "temperature_scaling",
                    predict_fn,
                ...
                corruption_results["temperature_scaling"] = {
                    "mce": None,
    ...
    def evaluate_sgc_method_faiss(
        method_key: str, method_data: dict, display_name: str = None
    ):
        ...
        try:
            if (
                ...
                logger.warning(
                    f"Cannot reconstruct {display_name} calibrator: missing train/val data"
                )
                return {"mce": None, "error": "missing train/val data"}

            # Extract layer information
            selected_layers = (
                method_data.get("selected_layers")
                or method_data.get("layer_names")
                or []
            )
            if not selected_layers:
                logger.warning(
                    f"No selected_layers/layer_names found for {display_name}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"FAISS backend not available for {display_name}, falling back to GeometricCalibrator"
                )
                use_faiss = False
            ...
            if use_faiss:
                ...
                geo_cal.fit(
                    X_val_embed=val_features, y_val=val_labels, X_val_original=val_raw
                )
    ...
    if "ablation" in results and isinstance(results["ablation"], dict):
        ...
        for method_key in ablation_methods:
            if method_key in results["ablation"]:
                ...
                result = evaluate_sgc_method_faiss(
                    method_key, results["ablation"][method_key], display_name
                )
                if result is not None:
...
def compute_ood_metrics(
    ...
    if ood_labels is not None:
        ...
        logger.info(
            f"  OOD Adaptive ECE: {ood_adaptive_ece:.4f} (lower = better calibration transfer)"
        )
        logger.info(
            f"  OOD Brier: {ood_brier:.4f} (lower = better calibration transfer)"
        )
...
def compute_baseline_ood_metrics(
    ...
    if baselines_section is None:
        logger.warning(
            "standard_baselines missing in results; skipping baseline OOD metrics"
        )
        ...
        if baselines_section["uncalibrated"].get("ood_auroc") is None:
            ...
            try:
                ...
                ood_metrics = compute_ood_metrics(
                    test_probs, ood_probs, test_labels, ood_labels=ood_labels
                )
        ...
        if not dac_result.get("skipped", False) and dac_result.get("ood_auroc") is None:
            ...
            try:
                if train_raw is None or train_labels is None:
                    logger.warning(
                        "train_raw and train_labels required for DAC OOD metrics, skipping"
                    )
                else:
                    from Calibrators.density_aware_calibration import (
                        DensityAwareCalibrator,
                    ...
                    val_loader = DataLoader(
                        TensorDataset(
                            torch.from_numpy(val_raw), torch.from_numpy(val_labels)
                        ),
                        batch_size=batch_size,
                        shuffle=False,
                    )
                    test_loader = DataLoader(
                        TensorDataset(
                            torch.from_numpy(test_raw), torch.from_numpy(test_labels)
                        ),
                        batch_size=batch_size,
                        shuffle=False,
                    )
                    ood_loader = DataLoader(
                        TensorDataset(
                            torch.from_numpy(ood_raw), torch.from_numpy(ood_labels)
                        ),
                        batch_size=batch_size,
                        shuffle=False,
                    )
                    ...
                    # Extract features
                    train_feats, _, train_lbls = extract_dac_features(
                        model, train_loader, layer_names, dev
                    )
                    val_feats, val_logits_dac, val_lbls = extract_dac_features(
                        model, val_loader, layer_names, dev
                    )
                    test_feats, test_logits_dac, _ = extract_dac_features(
                        model, test_loader, layer_names, dev
                    )
                    ood_feats, ood_logits_dac, _ = extract_dac_features(
                        model, ood_loader, layer_names, dev
                    )

                    # Fit DAC
                    dataset_name = (
                        model_adapter.dataset_name
                        if hasattr(model_adapter, "dataset_name")
                        else "cifar10"
                    )
                    ...
                    backends_to_run = (
                        ...
                        else [dac_knn_backend]
                    )

                    for backend in backends_to_run:
                        dac = make_dac_calibrator(
                            k=k_value, device=dev, backend=backend
                        )
                        ...
                        test_probs = dac.calibrate(
                            [f.numpy() for f in test_feats], test_logits_dac.numpy()
                        )
                        ood_probs = dac.calibrate(
                            [f.numpy() for f in ood_feats], ood_logits_dac.numpy()
                        )
                        ...
                        ood_metrics["knn_backend_impl"] = (
                            dac.__class__.__module__ + "." + dac.__class__.__name__
                        )
                        ...
                        if backend == "faiss" or dac_knn_backend == "faiss":
                            baselines_section["density_aware_calibration"].update(
                                ood_metrics
                            )
                        else:
                ...
                logger.error(
                    f"Failed to compute OOD for density_aware_calibration: {e}"
                )
                import traceback
    ...
    if "geometric_physical_space" in baselines_section:
        if baselines_section["geometric_physical_space"].get("ood_auroc") is None:
            ...
            try:
                if train_raw is None or train_labels is None:
                    logger.warning(
                        "train_raw and train_labels required for geometric_physical_space OOD metrics, skipping"
                    )
                else:
...
def save_per_sample_data(
    ...
    if model_name and dataset_name and training_method and seed is not None:
        out_dir = os.path.join(
            output_dir, model_name, dataset_name, training_method, str(seed)
        )
    else:
    ...
    per_sample_data = {
        ...
        "correct": (np.argmax(calibrated_probs, axis=1) == test_labels).astype(
            np.int8
        ),  # shape (n_test,)
    ...
    if model_name and dataset_name and training_method and seed is not None:
        ...
        safe_training_method = (
            training_method.replace(" ", "_").replace("/", "_").lower()
        )
...
def is_section_complete(
    results: Dict[str, Any], section_name: str, required_fields: List[str] = None
) -> bool:
    """
...
def should_rerun_section(
    ...
    if _FORCE_RERUN_GEOMETRIC:
        ...
        for geo_section in GEOMETRIC_SECTIONS_TO_RERUN:
            if (
                section_name == geo_section
                or section_name.startswith(geo_section + "_")
                or section_name.startswith(geo_section + ".")
            ):
                logger.info(
                    f"  {section_name}: force rerun (--force-rerun-geometric flag set)"
                )
                return True
    ...
    is_complete = is_section_complete(
        results, section_name, required_fields=required_fields
    )
...
def _convert_layer_metrics_nones_to_nan(
...
def _recompute_layer_metrics_for_patch(
    ...
    model = load_trained_model(
        model_path, args.model_name, num_classes, device, dataset=args.dataset
    )
...
def normalize_discovered_layers(
    model: torch.nn.Module, discovered: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
...
def filter_non_feature_layers(
    discovered_layers: List[Dict[str, Any]], model: torch.nn.Module
...
def calculate_adaptive_ece(
    ...
    bin_boundaries = np.interp(
        np.linspace(0, n_samples, n_bins + 1), np.arange(n_samples), sorted_confidences
    )
...
def validate_and_correct_coordinate_space(
    ...
    if corrections_made > 0:
        ...
        logger.info(
            f" All layer sizes validated. Total coordinate space: {total_size:,}"
        )
...
def build_depth_grouped_dac_features(
    ...
    if len(column_to_group) != features.shape[1]:
        logger.warning(
            f"Column-to-group mapping length ({len(column_to_group)}) != "
    ...
    logger.info(
        f"  Depth-based split: {group_sizes} coords per group (total {sum(group_sizes)})"
    )
...
def compute_layer_quality_metrics(
    ...
    val_np = (
        val_features.numpy() if isinstance(val_features, torch.Tensor) else val_features
    )

    # Convert to torch tensors for metric calculators
    train_torch = (
        torch.from_numpy(train_np)
        if isinstance(train_np, np.ndarray)
        else train_features
    )
    val_torch = (
        torch.from_numpy(val_np) if isinstance(val_np, np.ndarray) else val_features
    )
    ...
    if not isinstance(train_labels, torch.Tensor):
        train_labels_torch = (
            ...
            else torch.tensor(train_labels)
        )
    else:
    ...
    try:
        if precomputed_val_probs is not None:
            ...
            logger.info(
                f"  Using precomputed calibrated probabilities for calibration_decisiveness"
            )
            ...
            logger.info(
                f"  Calibrated decisiveness: {metrics['calibration_decisiveness']:.6f}"
            )
            ...
            val_np = (
                val_features.numpy() if torch.is_tensor(val_features) else val_features
            )
            ...
            calibrated_probs = geo_cal.calibrate_batched(
                X_test_embed=val_np, X_test_original=val_raw, batch_size=128
            )
            ...
            logger.warning(
                f"model_adapter or val_raw not provided, skipping calibration_decisiveness"
            )
            metrics["calibration_decisiveness"] = np.nan

    except Exception as e:
        logger.warning(
            f"Failed to compute calibration_decisiveness for {layer_name}: {e}"
        )
        import traceback
    ...
    try:
        ...
        metrics["multiscale_separation"] = multiscale_calc.compute(
            val_torch, val_labels_torch
        )
    except Exception as e:
    ...
    try:
        ...
        if len(train_unique_labels) < 2 or len(val_unique_labels) < 2:
            ...
            mu_c_list = []
            label_to_idx = {
                label.item(): idx for idx, label in enumerate(train_unique_labels)
            }
            ...
            for c in train_unique_labels:
                ...
                if len(class_features) == 0:
                    mu_c_list.append(
                        torch.zeros(train_torch.shape[1], device=train_torch.device)
                    )
                else:
...
def compute_all_layer_metrics_once(
    ...
    for i, layer_name in enumerate(layer_names):
        ...
        val_probs = None
        if (
            precomputed_calibrations is not None
            and layer_name in precomputed_calibrations
        ):
            if "val_calibrated_probs" in precomputed_calibrations[layer_name]:
                ...
                logger.info(
                    f"  Using precomputed calibrated probabilities (skipping redundant fitting)"
                )
...
def select_best_layer_by_metrics(
    ...
    for metric_name in metric_names:
        ...
        if include_median:
            ...
            median_layer = min(
                layer_values, key=lambda k: abs(layer_values[k] - median_val)
            )
            median_layer_val = layer_values[median_layer]
            selections[f"{metric_name} (Med)"] = median_layer

        logger.info(
            f"  {metric_name:25s} | Max: {max_layer:10s} ({max_val:.4f}) | Min: {min_layer:10s} ({min_val:.4f})"
            + (
                f" | Med: {median_layer:10s} ({median_layer_val:.4f})"
                if include_median
                else ""
            )
        )
    ...
    try:
        ...
        if not layers_with_nc:
            ...
            if not filtered_layers:
                ...
                psc_layer = max(
                    filtered_layers, key=lambda k: filtered_layers[k]["nc4"]
                )
                ...
                logger.info(
                    f"  Filtered {len(filtered_layers)}/{len(layers_with_nc)} layers with NC1 > {nc1_threshold}"
                )
                logger.info(
                    f"  Selected: {psc_layer} (NC1={psc_nc1:.4f}, NC4={psc_nc4:.4f})"
                )
        ...
        with torch.no_grad():
            for batch in dataloader:
                if len(batch) == 2:
                    ...
                    data, labels = batch[0], torch.zeros(
                        len(batch[0]), dtype=torch.long
                    )
...
def extract_features_from_multiple_layers(
    ...
    if not spp_only and compression_ratio is None and target_dimension is None:
        raise ValueError(
            "Either compression_ratio or target_dimension must be provided when spp_only=False"
        )
...
def get_layer_names_for_model(
    model_name: str, model: torch.nn.Module = None
) -> List[str]:
    """
    ...
    if "resnet" in model_name_lower:
        ...
        logger.info(
            f"  Total: {len(layer_names)} intermediate layers (logits added separately)"
        )
...
def get_penultimate_layer_name(model_name: str, all_layer_names: List[str]) -> str:
    ...
    if "resnet" in model_lower:
        ...
        candidates = [
            ...
            if "denseblock4" in n.lower() or "norm5" in n.lower()
        ]
        if candidates:
            return candidates[-1]
        ...
        candidates = [
            n for n in all_layer_names if "block" in n.lower() or "layer" in n.lower()
        ]
        if candidates:
            return candidates[-1]
        # Fallback: second-to-last layer
...
def extract_all_features_once(
    ...
    if target_dimension is not None:
        ...
        if dataset is None:
            ...
            logger.warning(
                f"Dataset not provided, using default compression_ratio: {compression_ratio}x"
            )
        else:
    ...
    test_loader = DataLoader(
        TensorDataset(
            torch.from_numpy(test_raw), torch.zeros(len(test_raw), dtype=torch.long)
        ),
        batch_size=batch_size,
        shuffle=False,
    )
...
def compute_all_separation_scores_once(
    ...
    compute_calibration_sep = (
        model_adapter is not None and val_raw is not None and test_raw is not None
    )
    if compute_calibration_sep:
        logger.info(
            "Computing BOTH oracle (true labels) and calibration (model predictions) separations"
        )
        ...
        logger.info(
            f"  Test accuracy: {100.0 * np.mean(test_pred == test_labels):.2f}%"
        )
    else:
    ...
    for i, layer_name in enumerate(layer_names):
        ...
        # Convert to numpy
        train_np = (
            train_features_per_layer[i].numpy()
            if isinstance(train_features_per_layer[i], torch.Tensor)
            else train_features_per_layer[i]
        )
        val_np = (
            val_features_per_layer[i].numpy()
            if isinstance(val_features_per_layer[i], torch.Tensor)
            else val_features_per_layer[i]
        )
        test_np = (
            test_features_per_layer[i].numpy()
            if isinstance(test_features_per_layer[i], torch.Tensor)
            else test_features_per_layer[i]
        )
        ...
        logger.info(
            f"    Oracle Val range: [{val_oracle_sep.min():.4f}, {val_oracle_sep.max():.4f}]"
        )
        ...
        if compute_calibration_sep:
            ...
            logger.info(
                f"    Calib Val range: [{val_calib_sep.min():.4f}, {val_calib_sep.max():.4f}]"
            )
...
def run_geometric_calibration(
    ...
    pooling_mode: str = "max",
    target_dimension: int = None,
    scoring_method: str = "separation",  # Separation scoring method
    ...
    ood_feature_cache: Dict[
        str, np.ndarray
    ] = None,  # OPTIMIZATION: Cache for OOD features
    ...
    """
    logger.info("\n" + "=" * 80)
    score_label = "Separation"
    logger.info(f"RUNNING GEOMETRIC CALIBRATION: {method_name} ({score_label})")
    logger.info("=" * 80)
    logger.info(
        "Feature preprocessing: SPP+JL compression with dataset-specific ratio (Geometric method)"
    )
    ...
    if precomputed_features is not None:
        ...
        if is_combined:
            ...
            filtered_layer_names = [
                name for name in layer_names if name in available_layer_set
            ]
            if len(filtered_layer_names) != len(layer_names):
                missing = set(layer_names) - available_layer_set
                logger.warning(
                    f"Some requested layers not found in precomputed features: {missing}"
                )
                logger.warning(
                    f"Using {len(filtered_layer_names)} available layers instead of {len(layer_names)} requested"
                )
            if not filtered_layer_names:
                raise ValueError(
                    f"None of the requested layers {layer_names} found in precomputed features. Available layers: {all_layer_names[:10]}..."
                )
            layer_indices = [
                all_layer_names.index(name) for name in filtered_layer_names
            ]
            ...
            logger.info(
                f"Combining {len(layer_indices)} requested layers from {len(all_layer_names)} available layers"
            )
            for idx, name in zip(layer_indices, layer_names):
                logger.info(
                    f"  Using layer {idx}: {name} (shape: {train_features_all[idx].shape})"
                )
            ...
            logger.info(
                f"Applying post-concatenation JL projection: {post_compression_ratio}x"
            )
        ...
        if is_combined:
            ...
            if target_dimension is not None:
                ...
                if dataset_lower == "cifar100":
                    ...
                    compression_ratio = 16.0  # Default for CIFAR10
                logger.info(
                    f"Using compression ratio: {compression_ratio}x for {dataset_name}"
                )
            ...
            train_features, _ = extract_features_directly(
                model, train_loader, layer_names[0], device
            )
            val_features, _ = extract_features_directly(
                model, val_loader, layer_names[0], device
            )
            test_features, _ = extract_features_directly(
                model, test_loader, layer_names[0], device
            )
    ...
    if return_val_probs:
        ...
        logger.info(
            f"  Validation calibrated probabilities shape: {val_calibrated_probs.shape}"
        )
    ...
    calibrator_throughput = (
        n_test_samples / calibrator_only_time if calibrator_only_time > 0 else 0.0
    )
    e2e_throughput = (
        n_test_samples / e2e_calibrate_time if e2e_calibrate_time > 0 else 0.0
    )
    ...
    if ood_raw is not None:
        ...
        ood_loader = DataLoader(
            TensorDataset(
                torch.from_numpy(ood_raw), torch.zeros(len(ood_raw), dtype=torch.long)
            ),
            batch_size=batch_size,
            shuffle=False,
        )
        ...
        if precomputed_features is not None:
            ...
            if is_combined:
                ...
                ood_features_list = extract_features_from_multiple_layers(
                    ...
                    compression_ratio=(
                        ...
                        if target_dimension is not None
                            ...
                            or dataset_name.lower() in ["tiny_imagenet", "tinyimagenet"]
                            else 16.0
                        )
                    ),
                    target_dimension=target_dimension,
                ...
                if compressor is None:
                    raise RuntimeError(
                        "compressor should have been created for combined layers with precomputed features"
                    )

                # Apply the same compression to OOD features (compressor is already fitted)
                ood_features_np = (
                    ood_features.numpy()
                    if torch.is_tensor(ood_features)
                    else ood_features
                )
                ood_features = torch.from_numpy(
                    compressor(ood_features_np, train=False)
                )
            else:
                # Single layer extraction
                ood_features, _ = extract_features_directly(
                    model, ood_loader, layer_names[0], device
                )
        else:
            ...
            if is_combined:
                ...
                if compressor is None:
                    raise RuntimeError(
                        "compressor should have been created for combined layers"
                    )
        ...
        # Convert to numpy
        ood_features_np = (
            ood_features.numpy() if torch.is_tensor(ood_features) else ood_features
        )
        ...
        if precomputed_ood_probs is not None:
            ...
            ood_model_probs = model_adapter.predict_proba(
                ood_raw, batch_size=batch_size
            )
        ...
        logger.info(
            f"  Confidence gap: {ood_metrics['confidence_gap']:.4f} (ID: {ood_metrics['id_confidence_mean']:.4f}, OOD: {ood_metrics['ood_confidence_mean']:.4f})"
        )
    ...
    results = {
        ...
        "calibrate_time_s": float(
            calibrate_time
        ),  # Backward compat: = calibrator_only_time
        ...
        "throughput_samples_per_sec": float(
            throughput
        ),  # Backward compat: = calibrator_throughput
...
def extract_and_aggregate_sgc_features(
    ...
    # Get dataset sizes without iterating loaders (handle None loaders for OOD case)
    def _safe_len_dataset(loader: Optional[DataLoader]) -> int:
        if loader is None:
            return 0
    ...
    n_train = _safe_len_dataset(train_loader)
    n_val = _safe_len_dataset(val_loader)
    n_test = _safe_len_dataset(test_loader)

    # Initialize accumulators ON DEVICE (GPU) (use empty tensors if loaders are None)
    train_sum = (
        torch.zeros((n_train, target_dim), device=device, dtype=torch.float32)
        if n_train > 0
        else torch.empty((0, target_dim), device=device, dtype=torch.float32)
    )
    val_sum = (
        torch.zeros((n_val, target_dim), device=device, dtype=torch.float32)
        if n_val > 0
        else torch.empty((0, target_dim), device=device, dtype=torch.float32)
    )
    test_sum = (
        torch.zeros((n_test, target_dim), device=device, dtype=torch.float32)
        if n_test > 0
        else torch.empty((0, target_dim), device=device, dtype=torch.float32)
    )
...
def run_global_random_calibration(
    ...
    use_precomputed = (
        precomputed_train_features is not None
        and precomputed_val_features is not None
        and precomputed_test_features is not None
        and precomputed_extraction_info is not None
        and precomputed_selected_layers is not None
    )
    ...
    if use_precomputed:
        ...
        filtered_layers, excluded_layers = filter_non_feature_layers(
            discovered_layers, model
        )
        all_layer_names = [d["name"] for d in filtered_layers]
        logger.info(
            f"Discovered {len(discovered_layers)} total layers, {len(all_layer_names)} feature-bearing layers after filtering"
        )
        ...
        if num_layers > len(all_layer_names):
            logger.warning(
                f"Requested {num_layers} but only {len(all_layer_names)} available"
            )
            selected_layers = all_layer_names
        else:
            selected_layers = list(
                np.random.choice(all_layer_names, size=num_layers, replace=False)
            )
        ...
        train_features, val_features, test_features, extraction_info = (
    ...
    if ood_raw is not None:
        ...
        if ood_loader is None:
            ood_loader = DataLoader(
                TensorDataset(
                    torch.from_numpy(ood_raw),
                    torch.zeros(len(ood_raw), dtype=torch.long),
                ),
                batch_size=batch_size,
                shuffle=False,
            )
...
# REMOVED: run_last_layer_only_baseline - Not used in paper
...
def run_sgc_with_dac_layers(
    ...
    logger.info("\n" + "=" * 80)
    logger.info(
        f"RUNNING SGC WITH DAC LAYER SELECTION (d={target_dim}) - {score_label}"
    )
    logger.info("=" * 80)
...
def run_sgc_with_tulip_layers(
    ...
    Args:
        ...
        output_file: Optional output file for incremental saving
        results_dict: Optional results dict for incremental saving
        scoring_method: 'separation' (only separation is used in the paper)
    ...
    logger.info("\n" + "=" * 80)
    logger.info(
        f"RUNNING SGC WITH TULIP LAYER SELECTION (d={target_dim}) - {score_label}"
    )
    logger.info("=" * 80)
...
def run_coordinate_calibration(
    ...
    logger.info("\n" + "=" * 80)
    logger.info(
        f"RUNNING COORDINATE SAMPLING CALIBRATION (K={num_coordinates}) - {score_label}"
    )
    logger.info("=" * 80)
    ...
    logger.info(
        f"Initial discovery: {total_size:,} scalars across {len(layer_map)} layers"
    )
    ...
    logger.info(
        f" Validated coordinate space: {total_size:,} scalars across {len(layer_map)} layers"
    )
    ...
    logger.info(
        f"  Top 5 layers by coordinate count: {sorted(coords_per_layer.items(), key=lambda x: -x[1])[:5]}"
    )
    ...
    logger.info(
        f"  Extracted features: train={train_features.shape}, val={val_features.shape}, test={test_features.shape}"
    )
    ...
    if ood_raw is not None:
        ...
        # Calibrate OOD using fitted calibrator
        ood_calibrated_probs = geo_cal.calibrate_batched(
            X_test_embed=ood_features_np, X_test_original=ood_raw, batch_size=batch_size
        )
...
# REMOVED: run_coordinate_spp_calibration - Not used in paper
# (Coordinate SPP experiment removed during code cleanup for paper submission)


# REMOVED: run_coordinate_spp_calibration and run_coordinate_spp_dac_calibration
# These experiments were not used in the paper and have been removed during cleanup.
...
def run_dac_with_coordinate_features(
    ...
    if (
        ...
        or sampling_plan is None
        or global_order is None
    ):
        # Determine input shape based on dataset
        is_dinov2 = model_name is not None and "dinov2" in model_name.lower()
        if is_dinov2:
    ...
    train_logits = (
        torch.cat(train_logits_list, dim=0)
        if train_logits_list
        else torch.empty((0,), dtype=torch.float32)
    )
    val_logits = (
        torch.cat(val_logits_list, dim=0)
        if val_logits_list
        else torch.empty((0,), dtype=torch.float32)
    )
    test_logits = (
        torch.cat(test_logits_list, dim=0)
        if test_logits_list
        else torch.empty((0,), dtype=torch.float32)
    )
    ...
    # Handle "both" mode: run both backends
    backends_to_run = (
        ["faiss", "torch"] if dac_knn_backend == "both" else [dac_knn_backend]
    )
    all_results = {}
    ...
    for backend in backends_to_run:
        ...
        if ood_raw is not None:
            ...
            logger.info(
                f"    Confidence gap ({backend}): {ood_metrics['confidence_gap']:.4f} (ID: {ood_metrics['id_confidence_mean']:.4f}, OOD: {ood_metrics['ood_confidence_mean']:.4f})"
            )
...
def run_geometric_with_dac_weighting(
    ...
    if precomputed_separations is not None:
        if "val_calibration_separation_matrix" in precomputed_separations:
            ...
            val_sep_matrix = precomputed_separations[
                "val_calibration_separation_matrix"
            ]
            test_sep_matrix = precomputed_separations[
                "test_calibration_separation_matrix"
            ]
        else:
            logger.warning(
        ...
        for i, layer_name in enumerate(all_layer_names):
            logger.info(
                f"  Computing separation for layer {i+1}/{n_layers}: {layer_name}"
            )

            # Convert to numpy
            train_np = (
                train_features_list[i].numpy()
                if isinstance(train_features_list[i], torch.Tensor)
                else train_features_list[i]
            )
            val_np = (
                val_features_list[i].numpy()
                if isinstance(val_features_list[i], torch.Tensor)
                else val_features_list[i]
            )
            test_np = (
                test_features_list[i].numpy()
                if isinstance(test_features_list[i], torch.Tensor)
                else test_features_list[i]
            )
            ...
            logger.info(
                f"    Val separation range: [{val_sep.min():.4f}, {val_sep.max():.4f}]"
            )
    ...
    logger.info(
        f"Optimizing {n_layers} layer weights + 1 bias using Squared Error Loss..."
    )
    ...
    if ood_raw is not None:
        ...
        for i, layer_name in enumerate(all_layer_names):
            ood_np = (
                ood_features_list[i].numpy()
                if isinstance(ood_features_list[i], torch.Tensor)
                else ood_features_list[i]
            )
        ...
        ood_calibrated_probs = ood_exp_logits / np.sum(
            ood_exp_logits, axis=1, keepdims=True
        )
...
def run_geometric_with_dac_features(
    ...
    if target_dimension is not None and original_dim > target_dimension:
        ...
        logger.info(
            f"  No compression needed (dim={original_dim}, target={target_dimension})"
        )
    ...
    start_calibrate = time.perf_counter()
    calibrated_probs = geo_cal.calibrate_batched(
        X_test_embed=test_features_np, X_test_original=test_raw, batch_size=batch_size
    )
    calibrate_time = time.perf_counter() - start_calibrate
...
def run_sgc_with_dac_preprocessing(
    ...
    logger.info("=" * 80)
    logger.info(
        "Testing: SGC pipeline with DAC preprocessing (spatial avg + L2) instead of SPP+JL"
    )
    ...
    if use_precomputed:
        ...
        for i in range(len(selected_layers)):
            train_feat = (
                train_features_all[i].numpy()
                if isinstance(train_features_all[i], torch.Tensor)
                else train_features_all[i]
            )
            val_feat = (
                val_features_all[i].numpy()
                if isinstance(val_features_all[i], torch.Tensor)
                else val_features_all[i]
            )
            test_feat = (
                test_features_all[i].numpy()
                if isinstance(test_features_all[i], torch.Tensor)
                else test_features_all[i]
            )
        ...
        if target_dim is not None and original_dim > target_dim:
            logger.info(
                f"4. Applying JL projection (Torch, GPU): {original_dim} -> {target_dim} dims"
            )
            ...
            logger.info(
                f"  No compression needed (dim={original_dim}, target={target_dim})"
            )
    ...
    start_calibrate = time.perf_counter()
    calibrated_probs = geo_cal.calibrate_batched(
        X_test_embed=test_features, X_test_original=test_raw, batch_size=batch_size
    )
    calibrate_time = time.perf_counter() - start_calibrate
    ...
    if ood_raw is not None:
        ...
        for i in range(len(selected_layers)):
            ood_feat = (
                ood_features_all[i].numpy()
                if isinstance(ood_features_all[i], torch.Tensor)
                else ood_features_all[i]
            )
        ...
        if target_dim is not None and ood_features_concat.shape[1] > target_dim:
            logger.info(
                f"  Applying JL projection to OOD features (Torch, GPU): {ood_features_concat.shape[1]} -> {target_dim} dims"
            )
        ...
        # Calibrate OOD using fitted calibrator
        ood_calibrated_probs = geo_cal.calibrate_batched(
            X_test_embed=ood_features, X_test_original=ood_raw, batch_size=batch_size
        )
...
def run_random_single_layer_with_dac_preprocessing(
    ...
    logger.info("=" * 80)
    logger.info(
        "Testing: Single random layer with DAC preprocessing + Geometric scoring"
    )
    ...
    # NOT L2 normalization. We apply L2 normalization here to match DAC's preprocessing pipeline.
    train_features = (
        train_features_all[0].numpy()
        if isinstance(train_features_all[0], torch.Tensor)
        else train_features_all[0]
    )
    val_features = (
        val_features_all[0].numpy()
        if isinstance(val_features_all[0], torch.Tensor)
        else val_features_all[0]
    )
    test_features = (
        test_features_all[0].numpy()
        if isinstance(test_features_all[0], torch.Tensor)
        else test_features_all[0]
    )
    ...
    rp_jl: Optional[TorchGaussianRandomProjectionJL] = (
        None  # JL projector reused for OOD
    )
    if target_dim is not None and original_dim > target_dim:
        logger.info(
            f"3. Applying JL projection (Torch, GPU): {original_dim} -> {target_dim} dims"
        )
        rp_jl = TorchGaussianRandomProjectionJL(
            in_dim=original_dim,
            out_dim=target_dim,
    ...
    if ood_raw is not None:
        ...
        # Get single layer features
        ood_features = (
            ood_features_all[0].numpy()
            if isinstance(ood_features_all[0], torch.Tensor)
            else ood_features_all[0]
        )
...
def run_dac_with_sgc_aggregated_features(
    ...
    logger.info("=" * 80)
    logger.info(
        "Testing: DAC algorithm with SGC's aggregated (project-then-sum) features"
    )
    logger.info(
        "NOTE: This gives DAC only 2 inputs (aggregated vector + logits), not a fair comparison"
    )
    ...
    val_labels = (
        val_labels_torch.numpy()
        if isinstance(val_labels_torch, torch.Tensor)
        else val_labels_torch
    )
    test_labels = (
        test_labels_torch.numpy()
        if isinstance(test_labels_torch, torch.Tensor)
        else test_labels_torch
    )
...
def run_dac_with_random_layer_selection(
    ...
    logger.info("=" * 80)
    logger.info(
        "Testing: DAC algorithm with randomly selected layers (fair comparison to SGC)"
    )
    ...
    for layer_idx, layer_name in enumerate(selected_layers):
        logger.info(
            f"  Extracting from layer {layer_idx+1}/{len(selected_layers)}: {layer_name}"
        )
    ...
    logger.info(
        f"DAC input: {len(train_features_for_dac)} layers ({len(selected_layers)} feature layers + 1 logits layer)"
    )
    logger.info(
        f"  DAC will learn {len(train_features_for_dac)} weights (one per layer + bias)"
    )
    ...
    for backend in backends_to_run:
        ...
        dac = make_dac_calibrator(k=k_value, device=device, backend=backend)
        dac.fit(
            train_features_for_dac, val_features_for_dac, val_logits_final, val_labels
        )
        ...
        if ood_raw is not None:
            ...
            ood_features_for_dac = [
                f.numpy() if isinstance(f, torch.Tensor) else f
                for f in ood_features_list
            ] + [ood_logits.numpy()]

            # Calibrate using fitted DAC
            ood_calibrated_probs = dac.calibrate(
                ood_features_for_dac, ood_logits.numpy()
            )
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=128, help="Batch size for data loading"
    )
    parser.add_argument(
        "--spp-pooling-mode",
        type=str,
    ...
    METHOD_CONFIGS = {
        ...
        # REMOVED: last_layer_only_baseline - not in paper
        "global_random_separation": {
            "section": "global_random_separation",
            "requires_features": True,
        },
        "sgc_faiss": {"section": "sgc_faiss", "requires_features": True},
        "coordinate_sampling_separation": {
            "section": "coordinate_sampling_separation",
            "requires_features": False,  # Uses coordinate extraction
        },
        # REMOVED: coordinate_spp_separation, coordinate_spp_faiss, coordinate_spp_dac - not in paper
        "dac_with_coordinate_features": {
    ...
    if args.method is not None and len(args.method) > 0:
        ...
        if invalid_methods:
            parser.error(
                f"Invalid method names: {invalid_methods}. Valid methods: {', '.join(METHOD_CONFIGS.keys())}"
            )
        selected_methods = set(args.method)
        logger.info("=" * 80)
        logger.info(
            f"METHOD SELECTION MODE: Running only {len(selected_methods)} specified method(s)"
        )
        ...
        needs_features = any(
            METHOD_CONFIGS[m]["requires_features"] for m in selected_methods
        )
        needs_dac_layers = any(
            METHOD_CONFIGS[m].get("requires_dac_layers", False)
            for m in selected_methods
        )
        needs_tulip_layers = any(
            METHOD_CONFIGS[m].get("requires_tulip_layers", False)
            for m in selected_methods
        )
        needs_coordinate_extraction = any(
            m.startswith("coordinate") or m == "dac_with_coordinate_features"
            for m in selected_methods
        )
    ...
    if is_dinov2:
        logger.info("=" * 80)
        logger.info(
            "DINOv2 detected: Skipping DAC experiments requiring architecture-specific layer selection"
        )
        logger.info(
            "  (DAC paper did not specify layers for transformer architectures)"
        )
    ...
    if args.ood_only:
        if not os.path.exists(output_file):
            logger.error(
                f"Cannot run --ood-only: results file not found at {output_file}"
            )
            return
        ...
        logger.info(
            f"Note: For geometric/DAC methods, OOD metrics will be computed when those sections are rerun"
        )
        return
    ...
    if args.force_rerun_geometric:
        logger.info("=" * 80)
        logger.info(
            "FORCE RE-RUN MODE: Re-running all geometric calibration experiments"
        )
        logger.info(
            "Reason: Previous runs used buggy percentile normalization (now fixed to rank-based)"
        )
        logger.info("=" * 80)
    ...
    if args.eval_corruptions or args.mce_only:
        ...
        if args.dataset.lower() not in ["cifar10", "cifar100"]:
            logger.error(
                f"Corruption evaluation only supported for CIFAR-10/100, got {args.dataset}"
            )
            return
        ...
        if os.path.exists(output_file) and not args.force_recompute:
            ...
            if "corruption_evaluation" in results:
                logger.info(
                    "Corruption evaluation already computed. Use --force-recompute to rerun."
                )
                return
        else:
            results = {}
        ...
        if "sgc_faiss" not in results or results.get("sgc_faiss", {}).get("error"):
            ...
            try:
                ...
                # Discover layers
                is_dinov2 = (
                    args.model_name is not None and "dinov2" in args.model_name.lower()
                )
                if is_dinov2:
                    input_shape = (1, 3, 224, 224)
                ...
                discovered_layers = normalize_discovered_layers(
                    ...
                    discover_model_layers(
                        model, device=device, input_shape=input_shape
                    ),
                )
                ...
                if num_layers > len(all_layer_names):
                    ...
                    selected_layers = list(
                        np.random.choice(
                            all_layer_names, size=num_layers, replace=False
                        )
                    )

                logger.info(
                    f"Selected {len(selected_layers)} layers: {selected_layers}"
                )
                ...
                # Get predictions
                val_probs = model_adapter.predict_proba(
                    val_raw, batch_size=args.batch_size
                )
                test_probs = model_adapter.predict_proba(
                    test_raw, batch_size=args.batch_size
                )
                val_predictions = np.argmax(val_probs, axis=1)
                test_predictions = np.argmax(test_probs, axis=1)
    ...
    if os.path.exists(output_file) and not args.force_recompute:
        ...
        if args.force_rerun_geometric:
            logger.info(
                "Deleting geometric calibration sections from loaded results..."
            )
            deleted_count = 0
            ...
            def is_geometric_section(section_name):
                ...
                for geo_section in GEOMETRIC_SECTIONS_TO_RERUN:
                    ...
                    if geo_section.startswith("ablation.") and section_name.startswith(
                        "ablation."
                    ):
                        ablation_key = geo_section.replace("ablation.", "")
                        if (
                            section_name == geo_section
                            or section_name.endswith("." + ablation_key)
                            or section_name.endswith("." + ablation_key + "_")
                        ):
                            return True
                return False
    ...
    if args.skip_single_layers:
        needs_model = (
            not is_section_complete(results, "standard_baselines")
            or (
                not is_dinov2
                and should_rerun_section(
            ...
            or should_rerun_section(
                results, "ablation.dac_with_random_layer_selection", DAC_REQUIRED_FIELDS
            )
            or (
                not is_dinov2
                and should_rerun_section(
            ...
            or (
                not is_dinov2
                and should_rerun_section(
                    results, "sgc_with_dac_layers", GEO_REQUIRED_FIELDS
                )
            )
            or should_rerun_section(
                results, "geometric_concatenated", GEO_REQUIRED_FIELDS
            )
            or should_rerun_section(results, "global_random", GEO_REQUIRED_FIELDS)
            or should_rerun_section(results, "coordinate_sampling", GEO_REQUIRED_FIELDS)
            or should_rerun_section(results, "coordinate_spp", GEO_REQUIRED_FIELDS)
            or should_rerun_section(results, "coordinate_spp_dac", GEO_REQUIRED_FIELDS)
            or should_rerun_section(
                results, "dac_with_coordinate_features", DAC_REQUIRED_FIELDS
            )
            or should_rerun_section(results, "dac_orig", DAC_REQUIRED_FIELDS)
            or not is_section_complete(results, "analysis", ["best_overall_method"])
        )
    else:
        needs_model = (
            not is_section_complete(results, "standard_baselines")
            or should_rerun_section(results, "geometric_original", GEO_REQUIRED_FIELDS)
            or (
                not is_dinov2
                and should_rerun_section(
                    results, "geometric_dac_weighted", DAC_REQUIRED_FIELDS
            ...
            or should_rerun_section(
                results, "metric_guided_calibration", GEO_REQUIRED_FIELDS
            )
            or should_rerun_section(
                results, "geometric_precomputed", GEO_REQUIRED_FIELDS
            )
            or should_rerun_section(
                results, "geometric_concatenated", GEO_REQUIRED_FIELDS
            )
    ...
    # Training set
    prediction_cache["train_probs"] = model_adapter.predict_proba(
        train_raw, batch_size=args.batch_size
    )
    prediction_cache["train_preds"] = np.argmax(prediction_cache["train_probs"], axis=1)

    # Validation set
    prediction_cache["val_probs"] = model_adapter.predict_proba(
        val_raw, batch_size=args.batch_size
    )
    prediction_cache["val_preds"] = np.argmax(prediction_cache["val_probs"], axis=1)

    # Test set
    prediction_cache["test_probs"] = model_adapter.predict_proba(
        test_raw, batch_size=args.batch_size
    )
    ...
    if ood_raw is not None:
        prediction_cache["ood_probs"] = model_adapter.predict_proba(
            ood_raw, batch_size=args.batch_size
        )
    ...
    dataloader_cache["test"] = DataLoader(
        TensorDataset(
            torch.from_numpy(test_raw), torch.from_numpy(test_labels.astype(np.int64))
        ),
        batch_size=args.batch_size,
        shuffle=False,
    )
    if ood_raw is not None:
        dataloader_cache["ood"] = DataLoader(
            TensorDataset(
                torch.from_numpy(ood_raw),
                torch.from_numpy(np.zeros(len(ood_raw), dtype=np.int64)),
            ),
            batch_size=args.batch_size,
            shuffle=False,
        )
    ...
    def get_or_extract_ood_features(
        cache_key: str, extract_fn, *args, **kwargs
    ) -> np.ndarray:
    ...
    if methods_only_mode:
        ...
        if is_dinov2:
            logger.info(
                "Skipping DAC in standard baselines (DINOv2: DAC paper did not specify layers for transformer architectures)"
            )
        ...
        if "density_aware_calibration" in baseline_results and not baseline_results[
            "density_aware_calibration"
        ].get("skipped", False):
    ...
    if args.eval_ensemble:
        if "deep_ensemble" not in results or should_rerun_section(
            results, "deep_ensemble", ["ece"]
        ):
            logger.info("\n" + "=" * 80)
            ...
            ensemble_seeds = [
                int(s.strip()) for s in args.ensemble_seeds.split() if s.strip()
            ]
            if not ensemble_seeds:
                logger.warning(
                    "No valid ensemble seeds provided, using default [11, 12, 13]"
                )
    ...
    if (
        args.corruption_type is None
        and uncalibrated_accuracy is not None
        and uncalibrated_accuracy < 0.5
    ):
        ...
        logger.info(
            f"Corruption experiment: skipping accuracy check (accuracy may be lower on corrupted data)"
        )

    if uncalibrated_accuracy is not None:
        logger.info(
            f"Model accuracy: {uncalibrated_accuracy:.2f}% - proceeding with calibration"
        )
    else:
    ...
    if methods_only_mode:
        ...
        needs_features = should_rerun_section(
            results, "geometric_dac_weighted", DAC_REQUIRED_FIELDS
        ) or should_rerun_section(
            results, "geometric_concatenated", GEO_REQUIRED_FIELDS
        )
        ...
        needs_features = (
            should_rerun_section(results, "geometric_original", GEO_REQUIRED_FIELDS)
            or should_rerun_section(
                results, "geometric_dac_weighted", DAC_REQUIRED_FIELDS
            )
            or should_rerun_section(
            ...
            or (
                "layer_quality_metrics" not in results
                or not results["layer_quality_metrics"]
            )
        )
    ...
    if needs_features:
        ...
        precomputed_separations = compute_all_separation_scores_once(
            precomputed_features["train_features_per_layer"],
            train_labels,
            precomputed_features["val_features_per_layer"],
            val_labels,
            precomputed_features["test_features_per_layer"],
            test_labels,
            all_layer_names,
            use_cuda=(device.type == "cuda"),
            model_adapter=model_adapter,
        ...
        logger.info(
            " All geometric experiments already computed, skipping feature extraction"
        )
    ...
    if methods_only_mode:
        ...
        if "metric_guided_calibration" not in results:
            results["metric_guided_calibration"] = {
                "skipped": True,
                "reason": "methods-only mode",
            }
        if "geometric_precomputed" not in results:
            results["geometric_precomputed"] = {
                "skipped": True,
                "reason": "methods-only mode",
            }
        save_results_incrementally(results, output_file)
        ...
        logger.info("\n" + "=" * 80)
        logger.info(
            "SKIPPING GEOMETRIC SINGLE-LAYER CALIBRATIONS (--skip-single-layers flag set)"
        )
        logger.info("=" * 80)
        logger.info(
            "--skip-single-layers: Also skipping layer_quality_metrics, metric_guided_calibration, geometric_precomputed"
        )
        # Initialize empty results for skipped sections
        if "geometric_original" not in results:
            results["geometric_original"] = {
                "skipped": True,
                "reason": "--skip-single-layers flag",
            }
        if "layer_quality_metrics" not in results:
        ...
        for i, layer_name in enumerate(all_layer_names, 1):
            ...
            if is_result_complete(existing, GEO_REQUIRED_FIELDS):
                logger.info(
                    f" Skipping layer {i}/{len(all_layer_names)}: {layer_name} (already has calibrator_params)"
                )
                continue
    ...
    if methods_only_mode:
        ...
        if needs_metrics:
            ...
            layer_metrics = {
                layer_name: {
                    k: (np.nan if v is None else float(v)) for k, v in metrics.items()
                }
    ...
    if methods_only_mode:
        ...
        needs_sgc_dac_preprocessing = (
            "sgc_with_dac_preprocessing_separation" in selected_methods
        )
        ...
        needs_dac_sgc_aggregated = should_rerun_section(
            results, "ablation.dac_with_sgc_aggregated_features", DAC_REQUIRED_FIELDS
        )
        needs_dac_random = should_rerun_section(
            results, "ablation.dac_with_random_layer_selection", DAC_REQUIRED_FIELDS
        )
        needs_sgc_dac_preprocessing = should_rerun_section(
            results, "ablation.sgc_with_dac_preprocessing", GEO_REQUIRED_FIELDS
        )
        needs_sgc_dac_layers = should_rerun_section(
            results, "ablation.sgc_with_dac_layers", GEO_REQUIRED_FIELDS
        )
        needs_sgc_tulip_layers = should_rerun_section(
            results, "ablation.sgc_with_tulip_layers", GEO_REQUIRED_FIELDS
        )
        needs_random_single_dac = should_rerun_section(
            results,
            "ablation.random_single_layer_dac_preprocessing",
            GEO_REQUIRED_FIELDS,
        )
    ...
    if (
        needs_dac_sgc_aggregated
        or needs_dac_random
        or needs_sgc_dac_preprocessing
        or needs_sgc_dac_layers
        or needs_sgc_tulip_layers
        or needs_random_single_dac
    ):
        logger.info("\n" + "=" * 80)
        ...
        if methods_only_mode:
            ...
            if (
                args.dac_knn_backend == "both"
                and isinstance(dac_with_sgc_aggregated, dict)
                and "dac_with_sgc_aggregated_features_torch" in dac_with_sgc_aggregated
            ):
                results["ablation"]["dac_with_sgc_aggregated_features"] = (
                    dac_with_sgc_aggregated.get(
                        "dac_with_sgc_aggregated_features", dac_with_sgc_aggregated
                    )
                )
                results["ablation"]["dac_with_sgc_aggregated_features_torch"] = (
                    dac_with_sgc_aggregated.get(
                        "dac_with_sgc_aggregated_features_torch"
                    )
                )
            else:
                results["ablation"][
                    "dac_with_sgc_aggregated_features"
                ] = dac_with_sgc_aggregated
            save_results_incrementally(results, output_file)
        else:
            logger.info(" DAC with SGC aggregated features already computed, skipping")
            dac_with_sgc_aggregated = results["ablation"].get(
                "dac_with_sgc_aggregated_features"
            )
        ...
        if methods_only_mode:
            ...
            if (
                args.dac_knn_backend == "both"
                and isinstance(dac_with_random_layers, dict)
                and "dac_with_random_layer_selection_torch" in dac_with_random_layers
            ):
                results["ablation"]["dac_with_random_layer_selection"] = (
                    dac_with_random_layers.get(
                        "dac_with_random_layer_selection", dac_with_random_layers
                    )
                )
                results["ablation"]["dac_with_random_layer_selection_torch"] = (
                    dac_with_random_layers.get("dac_with_random_layer_selection_torch")
                )
            else:
                results["ablation"][
                    "dac_with_random_layer_selection"
                ] = dac_with_random_layers
            save_results_incrementally(results, output_file)
        else:
            logger.info(" DAC with random layer selection already computed, skipping")
            dac_with_random_layers = results["ablation"].get(
                "dac_with_random_layer_selection"
            )
        ...
        if is_dinov2 and needs_sgc_dac_preprocessing:
            ...
            section_key = "sgc_with_dac_preprocessing_separation"
            ...
            # Check if separation scoring method needs to run
            needs_any_dac_preprocessing = should_rerun_section(
                    results,
                "ablation.sgc_with_dac_preprocessing_separation",
                    GEO_REQUIRED_FIELDS,
            )
            ...
            if needs_any_dac_preprocessing:
                ...
                if args.num_layers > len(all_layer_names):
                    ...
                    shared_dac_preprocessing_selected_layers = list(
                        np.random.choice(
                            all_layer_names, size=args.num_layers, replace=False
                        )
                    )

                logger.info(
                    f"Selected {len(shared_dac_preprocessing_selected_layers)} layers: {shared_dac_preprocessing_selected_layers}"
                )
                ...
                logger.info("\n" + "=" * 80)
                logger.info(
                    "EXTRACTING SGC WITH DAC PREPROCESSING FEATURES (shared across scoring methods)"
                )
                logger.info("=" * 80)
                logger.info(
                    "Extracting shared features for calibration"
                )
                ...
                train_features_all, _, _ = extract_dac_features_memory_efficient(
                    model,
                    train_loader_dac,
                    shared_dac_preprocessing_selected_layers,
                    device,
                ...
                # Concatenate all layers
                logger.info("3. Concatenating layer features...")
                train_features_concat = np.concatenate(
                    train_features_normalized, axis=1
                )
                val_features_concat = np.concatenate(val_features_normalized, axis=1)
                test_features_concat = np.concatenate(test_features_normalized, axis=1)
                ...
                original_dim = train_features_concat.shape[1]
                if (
                    args.target_dimension is not None
                    and original_dim > args.target_dimension
                ):
                    logger.info(
                        f"4. Applying JL projection (Torch, GPU): {original_dim} -> {args.target_dimension} dims"
                    )
                    ...
                    shared_dac_preprocessing_train_features = rp.project_np(
                        train_features_concat
                    )
                    shared_dac_preprocessing_val_features = rp.project_np(
                        val_features_concat
                    )
                    shared_dac_preprocessing_test_features = rp.project_np(
                        test_features_concat
                    )
                else:
                    logger.info(
                        f"  No compression needed (dim={original_dim}, target={args.target_dimension})"
                    )
                ...
                # Final L2 normalization
                logger.info("5. Applying final L2 normalization...")
                shared_dac_preprocessing_train_features = l2_normalize_np(
                    shared_dac_preprocessing_train_features
                )
                shared_dac_preprocessing_val_features = l2_normalize_np(
                    shared_dac_preprocessing_val_features
                )
                shared_dac_preprocessing_test_features = l2_normalize_np(
                    shared_dac_preprocessing_test_features
                )

                extract_time = time.perf_counter() - start_extract
                logger.info(
                    f"Feature extraction completed. Time: {extract_time:.2f}s, Final shape: {shared_dac_preprocessing_train_features.shape}"
                )

            # Run calibration for each scoring method
            for scoring_method in ["separation"]:
                ...
                if methods_only_mode:
                    ...
                    should_run = should_rerun_section(
                        results, f"ablation.{section_key}", GEO_REQUIRED_FIELDS
                    )
                ...
                if should_run:
                    ...
                    logger.info(
                        f" SGC with DAC preprocessing ({score_label}) already computed, skipping"
                    )
        else:
        ...
        if methods_only_mode:
            ...
            results["ablation"][
                "random_single_layer_dac_preprocessing"
            ] = random_single_dac
            save_results_incrementally(results, output_file)
        else:
            logger.info(
                " Random single layer with DAC preprocessing already computed, skipping"
            )
    else:
    ...
    if methods_only_mode and "sgc_with_dac_layers_separation" not in selected_methods:
        ...
        for scoring_method in ["separation"]:
            ...
            results[section_key] = {
                "skipped": True,
                "reason": "methods-only mode: not selected",
            }
        save_results_incrementally(results, output_file)
    ...
    if is_dinov2:
        logger.info(
            "Skipping: SGC with DAC layers (DINOv2: DAC paper did not specify layers)"
        )
        for scoring_method in ["separation"]:
            section_key = f"sgc_with_dac_layers_{scoring_method}"
            results[section_key] = {
        ...
        if methods_only_mode:
            ...
            needs_extraction = any(
                should_rerun_section(
                    results, f"sgc_with_dac_layers_{sm}", GEO_REQUIRED_FIELDS
                )
                for sm in ["separation"]
            )
        ...
        if needs_extraction:
            ...
            try:
                ...
                logger.error(f"Failed to get DAC target layers: {e}")
                raise ValueError(
                    f"Could not determine DAC layers for {args.model_name}: {e}"
                )
            ...
            (
                shared_train_features,
                shared_val_features,
                shared_test_features,
                shared_extraction_info,
            ) = extract_and_aggregate_sgc_features(
                model=model,
                layer_names=selected_layers,
                train_loader=train_loader,
            ...
            logger.info(
                f"Feature extraction completed. Final feature shape: {shared_train_features.shape}"
            )
        ...
        for scoring_method in ["separation"]:
            ...
            if methods_only_mode:
                ...
                should_run = should_rerun_section(
                    results, section_key, GEO_REQUIRED_FIELDS
                )
            ...
            if should_run:
                ...
                try:
                    ...
                    save_results_incrementally(results, output_file)
                    logger.info(
                        f" SGC with DAC layers ({score_label}) completed: ECE={sgc_dac_layers_result.get('ece', 'N/A'):.4f}"
                    )
                except Exception as e:
                ...
                logger.info(
                    f" SGC with DAC layer selection ({score_label}) already computed, skipping"
                )
    ...
    if is_dinov2:
        logger.info(
            "Skipping: SGC with TULIP layers (DINOv2: TULIP is designed for ResNets)"
        )
        for scoring_method in ["separation"]:
            section_key = f"sgc_with_tulip_layers_{scoring_method}"
            results[section_key] = {
        ...
        logger.info(
            "Skipping: SGC with TULIP layers (DenseNet: TULIP is designed for ResNets)"
        )
        for scoring_method in ["separation"]:
            section_key = f"sgc_with_tulip_layers_{scoring_method}"
            results[section_key] = {
        ...
        if methods_only_mode:
            ...
            needs_extraction = any(
                should_rerun_section(
                    results, f"sgc_with_tulip_layers_{sm}", GEO_REQUIRED_FIELDS
                )
                for sm in ["separation"]
            )
        ...
        if needs_extraction:
            ...
            try:
                ...
                logger.error(f"Failed to get TULIP target layers: {e}")
                raise ValueError(
                    f"Could not determine TULIP layers for {args.model_name}: {e}"
                )
            ...
            (
                tulip_train_features,
                tulip_val_features,
                tulip_test_features,
                tulip_extraction_info,
            ) = extract_and_aggregate_sgc_features(
                model=model,
            ...
            logger.info(
                f"Feature extraction completed. Final feature shape: {tulip_train_features.shape}"
            )
        ...
        for scoring_method in ["separation"]:
            ...
            if should_run:
                ...
                try:
                    ...
                    save_results_incrementally(results, output_file)
                    logger.info(
                        f" SGC with TULIP layers ({score_label}) completed: ECE={sgc_tulip_result.get('ece', 'N/A'):.4f}"
                    )
                ...
                logger.info(
                    f" SGC with TULIP layer selection ({score_label}) already computed, skipping"
                )
    ...
    if methods_only_mode:
        ...
        logger.info("\n" + "=" * 80)
        logger.info(
            "SKIPPING METRIC-GUIDED LAYER SELECTION (--skip-single-layers flag set)"
        )
        logger.info("=" * 80)
        metric_guided_results = []
        if "metric_guided_calibration" not in results:
        ...
        for metric_name_key, selected_layer in metric_selections.items():
            ...
            if is_result_complete(existing, GEO_REQUIRED_FIELDS):
                logger.info(
                    f" Skipping metric {metric_name_key} (already has calibrator_params)"
                )
            ...
            if selected_layer in geo_results_dict:
                logger.info(
                    f"  Reusing precomputed calibration result for {selected_layer}"
                )
                ...
                result = run_geometric_calibration(
                    ...
                    device,
                    selected_layer,
                    f"Geometric ({metric_name_key} → {selected_layer})",
                    args.batch_size,
                    precomputed_features=precomputed_features,
                    seed=args.seed,
                    pooling_mode=args.spp_pooling_mode,
            ...
            result["metric_value"] = layer_metrics[selected_layer].get(
                base_metric_name, float("nan")
            )
    ...
    if methods_only_mode and "last_layer_only_baseline" not in selected_methods:
        ...
        logger.info("=" * 80)
        logger.info(
            "Testing if multi-layer aggregation adds value over single penultimate layer"
        )
    ...
    if (
        methods_only_mode
        and "global_random_separation" not in selected_methods
        and "sgc_faiss" not in selected_methods
    ):
        logger.info("\n" + "=" * 80)
        ...
        global_random_result = results.get(
            "global_random_separation", results.get("global_random", {})
        )
    else:
        if "global_random" not in results:
            results["global_random"] = {}
        ...
        if methods_only_mode:
            needs_any_global_random = (
                "global_random_separation" in selected_methods
            ) or ("sgc_faiss" in selected_methods)
        else:
        ...
        if needs_any_global_random:
            ...
            if num_layers > len(all_layer_names):
                ...
                shared_global_random_selected_layers = list(
                    np.random.choice(all_layer_names, size=num_layers, replace=False)
                )

            logger.info(
                f"Selected {len(shared_global_random_selected_layers)} layers: {shared_global_random_selected_layers}"
            )
            ...
            logger.info("\n" + "=" * 80)
            logger.info(
                "EXTRACTING SGC FEATURES (shared across global_random experiments)"
            )
            logger.info("=" * 80)
            logger.info(
                "This extraction will be reused for separation and sgc_faiss"
            )

            (
                shared_global_random_train_features,
                shared_global_random_val_features,
                shared_global_random_test_features,
                shared_global_random_extraction_info,
            ) = extract_and_aggregate_sgc_features(
                model=model,
            ...
            logger.info(
                f"Feature extraction completed. Final feature shape: {shared_global_random_train_features.shape}"
            )
        ...
        for scoring_method in ["separation"]:
            ...
            if should_run:
                ...
                logger.info(
                    f" Global Random calibration ({score_label}) already computed, skipping"
                )
    ...
    if methods_only_mode and "sgc_faiss" not in selected_methods:
        ...
        try:
            ...
            if shared_global_random_train_features is not None:
                ...
                extraction_time = shared_global_random_extraction_info.get(
                    "extraction_time_s", 0.0
                )
                logger.info(
                    f"Using pre-extracted features (shape: {train_sgc_features.shape})"
                )
            else:
                # Fallback: extract features if not already extracted
                logger.info(
                    "Extracting features for SGC-FAISS (shared features not available)"
                )

                # Check if we have layer selection info from global_random
                if "global_random_layer_selection" in results:
                    selected_layers = results["global_random_layer_selection"][
                        "selected_layers"
                    ]
                    logger.info(
                        f"Reusing layer selection from global_random: {len(selected_layers)} layers"
                    )
                else:
                    # Discover layers and select randomly (same as global_random)
                    is_dinov2 = (
                        args.model_name is not None
                        and "dinov2" in args.model_name.lower()
                    )
                    if is_dinov2:
                        input_shape = (1, 3, 224, 224)
                    elif args.dataset.lower() in ["tiny_imagenet", "tinyimagenet"]:
                ...
                start_extraction = time.perf_counter()
                train_sgc_features, val_sgc_features, test_sgc_features, _ = (
                    extract_and_aggregate_sgc_features(
                        model=model,
                        layer_names=selected_layers,
                        train_loader=train_loader_sgc,
            ...
            isotonic_params = {
                ...
                "y_min": (
                    float(iso.y_thresholds_[0])
                    if hasattr(iso, "y_thresholds_") and len(iso.y_thresholds_) > 0
                    else None
                ),
                "y_max": (
                ...
                "stability_p5": (
                    float(sgc_faiss_cal.stability_p5)
                    if sgc_faiss_cal.stability_p5 is not None
                    else None
                ),
                "stability_p95": (
                    float(sgc_faiss_cal.stability_p95)
                    if sgc_faiss_cal.stability_p95 is not None
                    else None
                ),
            }
    ...
    if not (
        methods_only_mode and "coordinate_sampling_separation" not in selected_methods
    ):
        for scoring_method in ["separation"]:
            section_key = f"coordinate_sampling_{scoring_method}"
            score_label = (
            ...
            if should_run:
                ...
                logger.info(
                    f" Coordinate sampling ({score_label}) already computed, skipping"
                )

    # For backward compatibility, set coordinate_result to separation version
    coordinate_result = results.get(
        "coordinate_sampling_separation", results.get("coordinate_sampling", {})
    )

    # REMOVED: COORDINATE SPP CALIBRATION - Not in paper

    # REMOVED: COORDINATE SAMPLING WITH FAISS BACKEND - Not in paper
    # REMOVED: COORDINATE SPP WITH FAISS BACKEND - Not in paper  
    # REMOVED: COORDINATE SPP DAC CALIBRATION - Not in paper
    ...
    if methods_only_mode and "dac_orig" not in selected_methods:
        ...
        try:
            ...
            train_feats_list, _, train_labels_dac = extract_dac_features(
                raw_model, tr_loader, layer_names, dev
            )
            val_feats_list, logits_val_dac, val_labels_dac = extract_dac_features(
                raw_model, va_loader, layer_names, dev
            )
            test_feats_list, logits_test_dac, _ = extract_dac_features(
                raw_model, te_loader, layer_names, dev
            )
            ...
            train_feats_list_np = [
                feat.detach().cpu().numpy() for feat in train_feats_list
            ]
            val_feats_list_np = [feat.detach().cpu().numpy() for feat in val_feats_list]
            test_feats_list_np = [
                feat.detach().cpu().numpy() for feat in test_feats_list
            ]
            ...
            for backend in backends_to_run:
                ...
                probs_dac = dac.calibrate(
                    test_features_list=test_feats_list_np, test_logits=test_logits_np
                )
                ...
                logger.info(
                    f"  DAC calibration time ({backend}): {dac_calibrate_elapsed:.2f}s"
                )

                # Compute end-to-end timing (for fair comparison with RGCL/RGCC)
                dac_offline_time = (
                    dac_extract_elapsed + dac_fit_elapsed
                )  # extraction + fit
                dac_end_to_end_time = (
                    dac_offline_time + dac_calibrate_elapsed
                )  # extraction + fit + calibrate
                logger.info(
                    f"  DAC offline time ({backend}): {dac_offline_time:.2f}s (extract + fit)"
                )
                logger.info(
                    f"  DAC end-to-end time ({backend}): {dac_end_to_end_time:.2f}s (extract + fit + calibrate)"
                )
                ...
                throughput = (
                    len(test_labels) / dac_calibrate_elapsed
                    if dac_calibrate_elapsed > 0
                    else float("inf")
                )
                ...
                result_dict = {
                    ...
                    "layer_weights": [1.0 / max(1, len(layer_names))]
                    * len(layer_names),
                    "method": "Original DAC (Density-Aware Calibration)",
                    "knn_backend": backend,
                    "knn_backend_impl": dac.__class__.__module__
                    + "."
                    + dac.__class__.__name__,
                }

                logger.info(f"  DAC Original ECE ({backend}): {ece:.6f}")
                logger.info(
                    f"  DAC Original Adaptive ECE ({backend}): {adaptive_ece:.6f}"
                )
                logger.info(f"  DAC Original Brier ({backend}): {brier:.6f}")
                logger.info(f"  DAC Original Accuracy ({backend}): {accuracy:.4f}%")
                logger.info(
                    f"  DAC Original Throughput ({backend}): {throughput:.1f} samples/sec"
                )
    ...
    if methods_only_mode and "dac_with_coordinate_features" not in selected_methods:
        ...
        if (
            args.dac_knn_backend == "both"
            and isinstance(dac_coord_result, dict)
            and "dac_with_coordinate_features_torch" in dac_coord_result
        ):
            results["dac_with_coordinate_features"] = dac_coord_result.get(
                "dac_with_coordinate_features", dac_coord_result
            )
            results["dac_with_coordinate_features_torch"] = dac_coord_result.get(
                "dac_with_coordinate_features_torch"
            )
        else:
    ...
    if args.skip_single_layers:
        logger.info("\n" + "=" * 80)
        logger.info(
            "SKIPPING PRE-COMPUTED THROUGHPUT EXPERIMENT (--skip-single-layers flag set)"
        )
        logger.info("=" * 80)
        if "geometric_precomputed" not in results:
            results["geometric_precomputed"] = {"skipped": True}
        ...
        geo_precomputed = run_geometric_calibration(
            ...
            device,
            best_layer_name,
            f"Geometric Pre-computed ({get_semantic_layer_name(best_layer_name, args.model_name)})",
            args.batch_size,
            precomputed_features=precomputed_features,
    ...
    if should_rerun_section(results, "geometric_concatenated", GEO_REQUIRED_FIELDS):
        ...
        if "resnet" in args.model_name.lower():
            ...
            main_blocks = [
                ...
                if "dense" in name.lower() and "trans" not in name.lower()
            ]
        ...
        if precomputed_features is not None:
            ...
            if not main_blocks:
                logger.warning(
                    f"No matching layers found in precomputed features. Available: {precomputed_features['layer_names'][:10]}..."
                )
                geo_combined = {
                ...
                logger.info(
                    f"Filtered main_blocks to {len(main_blocks)} layers that exist in precomputed features: {main_blocks}"
                )
        ...
        if geo_combined is None:
            try:
                geo_combined = run_geometric_calibration(
                    ...
                    device,
                    main_blocks,
                    f"Geometric (Combined {len(main_blocks)} Main Blocks)",
                    args.batch_size,
                    precomputed_features=precomputed_features,
                    seed=args.seed,
                    pooling_mode=args.spp_pooling_mode,
    ...
    if not is_section_complete(results, "analysis", ["best_overall_method"]):
        ...
        if args.skip_single_layers or not geo_results_dict:
            ...
            geo_results_list = [geo_results_dict[ln] for ln in all_layer_names]
            best_geo_single = (
                min(geo_results_list, key=lambda x: x["ece"])
                if geo_results_list
                else None
            )
                ...
                all_results_for_best.append(
                    {
                        ...
                        "accuracy": baselines["density_aware_calibration"].get(
                            "acc", 0
                        ),
                    }
                )
        ...
        if (
            "global_random" in results
            and results["global_random"].get("ece") is not None
        ):
            all_results_for_best.append(results["global_random"])
        if (
            "coordinate_sampling" in results
            and results["coordinate_sampling"].get("ece") is not None
        ):
            all_results_for_best.append(results["coordinate_sampling"])
        if (
            "coordinate_spp" in results
            and results["coordinate_spp"].get("ece") is not None
        ):
            all_results_for_best.append(results["coordinate_spp"])
        if (
            "coordinate_spp_dac" in results
            and results["coordinate_spp_dac"].get("ece") is not None
        ):
            all_results_for_best.append(results["coordinate_spp_dac"])
        if (
            "dac_with_coordinate_features" in results
            and results["dac_with_coordinate_features"].get("ece") is not None
        ):
        ...
        best_metric_guided = (
            min(metric_guided_with_ece, key=lambda x: x["ece"])
            if metric_guided_with_ece
            else None
        )
        ...
        if (
            dac_results
            and dac_results.get("ece") is not None
            and uncalibrated_results.get("ece") is not None
        ):
            dac_ece_improvement = (
                (uncalibrated_results["ece"] - dac_results["ece"])
                / uncalibrated_results["ece"]
            ) * 100
        best_overall_improvement = None
        if uncalibrated_results.get("ece") is not None:
            best_overall_improvement = (
                (uncalibrated_results["ece"] - best_overall["ece"])
                / uncalibrated_results["ece"]
            ) * 100
        ...
        if "last_layer_only_baseline" in results and "global_random" in results:
            ...
            if last_layer_ece is not None and sgc_random_ece is not None:
                ...
                improvement_percent = (
                    ((last_layer_ece - sgc_random_ece) / last_layer_ece) * 100
                    if last_layer_ece > 0
                    else 0.0
                )
                last_layer_vs_random_comparison = {
                    ...
                    "conclusion": (
                        "Multi-layer aggregation provides value"
                        if random_improves
                        else "Last layer sufficient"
                    ),
                }
                logger.info(f"\nLast Layer vs SGC Random Comparison:")
                ...
                logger.info(
                    f"  SGC improves: {random_improves} ({improvement_percent:.2f}%)"
                )
                logger.info(
                    f"  Conclusion: {last_layer_vs_random_comparison['conclusion']}"
                )
        ...
        if (
            "coordinate_sampling" in results
            and "dac_with_coordinate_features" in results
        ):
        ...
        if dac_results and dac_results.get("ece") is not None:
            improvement_over_dac_percent = float(
                ((dac_results["ece"] - best_overall["ece"]) / dac_results["ece"]) * 100
            )
        ...
        results["analysis"] = {
            ...
            "dac_ece_improvement_over_uncalibrated_percent": (
                float(dac_ece_improvement) if dac_ece_improvement is not None else None
            ),
            "best_overall_ece_improvement_over_uncalibrated_percent": (
                float(best_overall_improvement)
                if best_overall_improvement is not None
                else None
            ),
            "best_metric_guided_method": (
                best_metric_guided["method"] if best_metric_guided else None
            ),
            "best_metric_guided_ece": (
                float(best_metric_guided["ece"]) if best_metric_guided else None
            ),
            "best_metric_guided_selection_metric": (
                best_metric_guided["selection_metric"] if best_metric_guided else None
            ),
        ...
        if best_overall_improvement is not None:
            logger.info(
                f"Improvement over Uncalibrated: {best_overall_improvement:.2f}%"
            )
        else:
            logger.info(
                "Improvement over Uncalibrated: N/A (uncalibrated ECE not available)"
            )
    ...
    if args.eval_corruptions and args.dataset.lower() in ["cifar10", "cifar100"]:
        ...
        test_transform = None
        if hasattr(test_loader, "dataset") and hasattr(
            ...
            if args.dataset.lower() == "cifar10" or args.dataset.lower() == "cifar100":
                    ...
                    [
                        ...
                        transforms.Normalize(
                            mean=(
                                ...
                                if args.dataset.lower() == "cifar10"
                                else [0.5071, 0.4867, 0.4408]
                            ),
                            std=(
                                [0.2023, 0.1994, 0.2010]
                                if args.dataset.lower() == "cifar10"
                                else [0.2675, 0.2565, 0.2761]
                            ),
                        ),
                    ]
                )
                ...
                logger.warning(
                    f"Cannot determine test_transform for dataset {args.dataset}, skipping corruption evaluation"
                )
                test_transform = None
        ...
        if test_transform is not None:
            ...
            logger.warning(
                "Could not determine test_transform, skipping corruption evaluation"
            )
    elif args.eval_corruptions:
        logger.warning(
            f"Corruption evaluation only supported for CIFAR-10 and CIFAR-100, got {args.dataset}"
        )
