#!/usr/bin/env python3
"""
Generate (and optionally submit) SLURM jobs for training classification models.
...
Each job wraps `Experiments/train_model.py` for a single seed so we
can launch multiple seeds in parallel with consistent resource settings.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create/submit SLURM jobs for model training (multiple architectures and datasets)",
    ...
    parser.add_argument(
        "--train-script",
        type=Path,
        default=PROJECT_ROOT / "Experiments" / "train_model.py",
    ...
    parser.add_argument(
        "--download", action="store_true", help="Download CIFAR-10 if missing."
    )

    # SLURM settings
    parser.add_argument(
        "--partition", type=str, default="gpu", help="Partition / queue."
    )
    parser.add_argument(
        "--gpu-type",
        type=str,
        default="rtx_4090",
        help="GPU type (e.g., rtx_6000, rtx_4090).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--time", type=str, default="1-00:00:00", help="Wall clock limit (d-hh:mm:ss)."
    )
    parser.add_argument(
        "--conda-env",
        type=str,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for SLURM stdout logs. Defaults to results/logs/{model}_{dataset}.",
    )
    parser.add_argument(
        "--submit",
        action="store_true",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Print planned jobs and exit early."
    )
    parser.add_argument(
        "--sleep", type=float, default=0.25, help="Delay between submissions (seconds)."
    )
    parser.add_argument(
        "--env-extra",
        type=str,
...
def build_jobs(args: argparse.Namespace) -> List[TrainingJob]:
    ...
    for model_name in args.models:
        if model_name.startswith("dinov2") and model_name not in valid_dinov2:
            raise ValueError(
                f"Invalid DINOv2 model: {model_name}. Choose from {valid_dinov2}"
            )
    ...
    for model_name, dataset, seed in itertools.product(
        args.models, args.datasets, seeds
    ):
