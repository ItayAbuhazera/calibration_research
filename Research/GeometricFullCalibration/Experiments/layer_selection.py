...
def run_standard_baselines(model_adapter,
    ...
    try:
        ...
        try:
            ...
            probs_dac = None
            results["density_aware_calibration"] = {"ece": None, "brier": None, "acc": None, "error": f"Layer extraction failed: {e}", "timing_seconds": dac_elapsed}
        else:
            # Create data loaders
...
def screen_layers_with_meta_predictor(
    ...
    if will_work_layers:
        ...
        logger.warning("  No layers predicted to work well!")
...
def run_multi_composite_experiment(args):
    ...
    try:
        ...
        if test_acc_quick < ACCURACY_THRESHOLD:
            logger.warning(f"  Model accuracy ({test_acc_quick:.2%}) is below threshold ({ACCURACY_THRESHOLD:.0%})")
        ...
        logger.warning(f"  Could not perform early accuracy check: {e}")
    ...
    try:
        if getattr(args, "eval_corruptions", False):
            ...
            if geo_mce_per_layer_local:
                ...
                logger.info("Skipping per-layer mCE ground-truth save (no per-layer mCE available yet).")
    except Exception as e:
    ...
    try:
        if getattr(args, "eval_corruptions", False):
            ...
            if geo_mce_per_layer_local:
                ...
                logger.info("Skipping per-metric mCE artifacts (no per-layer mCE available).")
    except Exception as e:
