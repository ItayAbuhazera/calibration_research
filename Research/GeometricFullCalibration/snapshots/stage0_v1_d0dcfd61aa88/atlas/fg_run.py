...
def verify_spec_hash() -> str:
    """sha256 of the frozen part of the spec (text before END FROZEN) must equal the value recorded in results/fixed_gate/freeze_manifest.json."""
    h = hashlib.sha256(open("docs/fixed_gate_study_spec.md").read().split("<!-- END FROZEN -->")[0].encode()).hexdigest()
    rec = json.load(open("results/fixed_gate/freeze_manifest.json"))["spec_frozen_part_sha256"]
    if h != rec:
        raise SystemExit(f"frozen spec hash mismatch: {h} != {rec}")
    return h
...
def freeze(seed: int, root_out: str = None):
    ...
    payload = {"seed": seed, "spec_sha256": verify_spec_hash(), "gates": frozen,
