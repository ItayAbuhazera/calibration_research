...
## Results (appended 2026-09-21; not covered by the frozen hash)
Verified by `python -c` hash check against `results/fixed_gate/freeze_manifest.json`. Facts and tables: `results/fixed_gate/report/` and per-seed artifacts; summary in ResearchBrain `05_Experiments/2026-09-21 Fixed Deep Candidate Gate Study.md`.
Headline: deep Z1 − Z0 (n=2500) clean +0.10 / +0.13 pp, corruption macro −0.04 / −0.04 pp (intervals include 0); deep Z1 vs base macro +0.075 / +0.21 pp; C0 ≈ Z1; no growth with n or m; deep candidate 873/908 fit disagreements vs 144/69 (layer4) and 83/74 (output). Base-prediction mismatch with the benchmark is reproduced exactly by the benchmark path (TF32 convs, batch 128). Deferred: seed-4 reconciliation jobs 21537819/20/21.
