...
def get_data_loaders(
    ...
    preprocessing_protocol: None / "legacy_v1_mixed_norm" reproduces the
    historical CIFAR-100 clean-test normalization (ImageNet statistics, a
    verified train/test mismatch). "corrected_v2_train_norm" evaluates with
    the statistics the checkpoints were trained with. Only the CIFAR-100
    clean TEST loader differs between the two (train/validation loaders and
    CIFAR-10 / Tiny ImageNet are already consistent); see
    utils/preprocessing_protocol.py. Corruption loaders built here are the
    legacy dataset classes and are not affected -- the unified benchmark
    builds its own CIFAR-C loader from the same protocol object.
