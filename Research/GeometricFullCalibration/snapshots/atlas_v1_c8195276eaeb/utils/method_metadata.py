...
# FV-DAC (standalone study) splits the clean validation split into disjoint
# fit/select halves via utils.decision_audit.make_inner_validation_split, so
# it has a real selection split where the Phase 0/1 geometric methods do not.
_FV_DAC_HIDDEN_BUDGET = dict(
    logits=True, softmax=True, representation=True, labels_in_fitting=True,
    fitting_split="calibration_validation_inner_fit",
    selection_split="calibration_validation_inner_select",
)
_FV_DAC_LOGIT_SPACE_BUDGET = dict(
    logits=True, softmax=True, representation=False, labels_in_fitting=True,
    fitting_split="calibration_validation_inner_fit",
    selection_split="calibration_validation_inner_select",
)
...
_EXACT: Dict[str, Dict[str, Any]] = {
    ...
    # -- Full-Vector DAC (standalone FV-DAC study, 2026-09-20) ------------
    # These rows are ADDITIVE: no existing method's semantics change, and
    # Experiments/run_unified_benchmark.py does not emit any of them. They
    # are consumed by Experiments/run_fv_dac_experiment.py so its artifacts
    # go through the same canonical semantics registry as the benchmark's.
    # Every FV-DAC arm is full-vector, its own argmax IS the decision, and
    # it CAN change that decision -- that is the entire point of the study
    # (native_dac above is the argmax-invariant parent it extends).
    "fv_dac_nll": _m(
        output_semantics=FULL_VECTOR,
        effective_prediction_source=PRED_CALIBRATED,
        argmax_invariant=False, can_change_argmax=True,
        ...
        notes="FV-DAC primary: softmax((z - beta*R(x)) / S_DAC(x)) where R_k(x) is the "
              "native-DAC-weighted mean of class-conditioned K_c-th nearest-neighbour "
              "distances. beta >= 0 fitted by NLL on clean inner-fit data. NOT "
              "argmax-invariant: the correction is class-dependent, so the decision can "
              "change. beta=0 reproduces native_dac numerically.",
        information_budget=_budget(
            **_FV_DAC_HIDDEN_BUDGET,
            objective="clean inner-fit NLL",
            trainable_params="1 (beta); native DAC's L+1 weights are frozen inputs",
            hp_search_budget="3 predeclared K_c values, selected on a disjoint clean split",
        ),
    ),
    "fv_dac_brier": _m(
        output_semantics=FULL_VECTOR,
        ...
        metric_bucket=FULL_VECTOR_BUCKET,
        sample_dep=True, geometry_dep=True, class_dep=True, joint=True,
        notes="FV-DAC objective sensitivity: identical to fv_dac_nll except beta is fitted "
              "with the squared-error objective native DAC itself uses. Predeclared "
              "sensitivity, never the primary arm.",
        information_budget=_budget(
            **_FV_DAC_HIDDEN_BUDGET,
            objective="clean inner-fit squared error (DAC's own objective)",
            trainable_params="1 (beta)", hp_search_budget="inherits the primary K_c",
        ),
    ),
    "fv_dac_lognorm": _m(
        output_semantics=FULL_VECTOR,
        ...
        metric_bucket=FULL_VECTOR_BUCKET,
        sample_dep=True, geometry_dep=True, class_dep=True, joint=True,
        notes="FV-DAC conditioning sensitivity A: the class distances are replaced by "
              "v_{l,k} = -(log r_{l,k} - mu_{l,k}) / sigma_l with mu/sigma frozen from "
              "clean fitting data, then softmax((z + beta*V)/S_DAC). Predeclared "
              "sensitivity, not the canonical extension.",
        information_budget=_budget(
            **_FV_DAC_HIDDEN_BUDGET,
            objective="clean inner-fit NLL",
            trainable_params="1 (beta); mu/sigma are frozen statistics, not fitted "
                             "decision parameters",
            hp_search_budget="inherits the primary K_c",
        ),
    ),
    "fv_dac_shared_layer": _m(
        output_semantics=FULL_VECTOR,
        effective_prediction_source=PRED_CALIBRATED,
        argmax_invariant=False, can_change_argmax=True,
        metric_bucket=FULL_VECTOR_BUCKET,
        sample_dep=True, geometry_dep=True, class_dep=True, joint=True,
        notes="FV-DAC sensitivity B: softmax((z - sum_l b_l r_{l,.}) / S_DAC) with b_l >= 0 "
              "SHARED across classes (O(L) parameters, never b_{l,k}). Tests whether DAC's "
              "calibration-optimal layer weighting is also decision-optimal.",
        information_budget=_budget(
            **_FV_DAC_HIDDEN_BUDGET,
            objective="clean inner-fit NLL",
            trainable_params="L (one non-negative weight per DAC layer, class-symmetric)",
            hp_search_budget="inherits the primary K_c",
        ),
    ),
    "fv_dac_permuted": _m(
        ...
        metric_bucket=FULL_VECTOR_BUCKET,
        sample_dep=True, geometry_dep=True, class_dep=True, joint=True,
        notes="FV-DAC NEGATIVE CONTROL: identical to fv_dac_nll with a seeded sample-level "
              "permutation of the reference-bank labels. Same features, same distance code, "
              "same dimensionality, same K_c, same beta protocol; only the geometry-to-class "
              "correspondence is destroyed. Expected null: beta -> 0 or no decision gain.",
        information_budget=_budget(
            **_FV_DAC_HIDDEN_BUDGET,
            objective="clean inner-fit NLL",
            trainable_params="1 (beta)", hp_search_budget="inherits the primary K_c",
    ...
    "fv_dac_logit_space": _m(
        output_semantics=FULL_VECTOR,
        effective_prediction_source=PRED_CALIBRATED,
        argmax_invariant=False, can_change_argmax=True,
        metric_bucket=FULL_VECTOR_BUCKET,
        sample_dep=True, geometry_dep=False, class_dep=True, joint=True,
        notes="FV-DAC MECHANISM CONTROL in output space: the same class-conditioned operator "
              "on centered, L2-normalized logits (z - mean(z)), same bank membership, labels, "
              "K_c and beta protocol; S_DAC stays the hidden-space scalar. "
              "representation_geometry_dependent=False because the GEOMETRY is built from "
              "logits, not hidden layers -- S_DAC is a frozen scalar input, not this arm's "
              "own geometric signal. NOT capacity-matched to the hidden arm (1 source of "
              "dim 100 vs 5 sources of dim 64/256/512/1024/2048).",
        information_budget=_budget(
            **_FV_DAC_LOGIT_SPACE_BUDGET,
            objective="clean inner-fit NLL",
            trainable_params="1 (beta)", hp_search_budget="inherits the primary K_c",
        ),
    ),
