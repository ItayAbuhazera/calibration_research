...
class BasicBlock(nn.Module):
    ...
    def forward_with_features(self, x):
        """
        Forward pass that returns both logits and features
        Features are extracted right before the final FC layer
        """
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        ...
        # These are our features (before FC layer)
        features = out.view(out.size(0), -1)
        
        # Apply FC layer for logits
...
class ResNet(nn.Module):
    ...
    def forward_with_features(self, x):
        ...
        logits = self.fc(feature_vector) / self.temp
