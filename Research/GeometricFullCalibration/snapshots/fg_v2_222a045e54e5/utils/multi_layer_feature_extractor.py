...
class LayerFeatureExtractor:
        ...
        for layer_name in self.layer_names:
            if layer_name in self.layer_features:
                ...
                if features.size(0) == batch_size:
                    ...
                    logger.warning(f" Size mismatch for {layer_name}: "
    ...
    def finalize_layer_selection(self) -> Optional[str]:
        ...
        for layer_name in self.layer_names:
            ...
            if (layer_name not in self.collection_data['features'] or 
                ...
                logger.info(f"Skipping {layer_name}: no data collected")
                continue
            ...
            try:
                ...
                if norm_range < 1e-10:
                    logger.warning(f"    Features have uniform magnitude - using default confidence")
                ...
                if np.isnan(ece) or np.isinf(ece) or ece < 0 or ece > 1:
                    logger.warning(f"    Invalid ECE value: {ece}")
                    failed_evaluations += 1
                    continue
        ...
        if best_layer:
            ...
            logger.warning(f"    No valid layers found - all evaluations failed")
    ...
    def _register_hooks(self):
        ...
        logger.info(f"    Detected architecture: {architecture_type}")
