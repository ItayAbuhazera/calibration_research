...
| Source-recoverability of intermediate-layer evidence is controlled by its clean redundancy given Z (clean redundancy manipulated by frozen-vs-fine-tuned ImageNet-pretrained ResNet-50) | [[2026-09-24 Regime-Map Pilot]] | Frozen spec v2, two fine-tuning seeds plus one frozen state: row 4 fired in both seeds, i.e. the claim is **killed for this evidence**; the recoverability gap is present in all seven states (8k×1 gap +1.53 [+1.26, +1.78] to +2.77 [+2.43, +3.09] in the extremes read from the artifact) and is larger in the clean-complementary frozen state than after fine-tuning | A cause for the gap; that clean redundancy never matters; other backbones/resolutions/layers/readouts; anything deployable. **Δ_T uses target labels (oracle ceiling, not deployable)** and the root cause is unresolved | `results/regime_map/report/regime_aggregate.json`, `docs/regime_map_pilot_spec_v2.md` | closed for the tested claim (row 4, both seeds); gap cause open; a capacity-control follow-up is proposed but **not authorized** |
