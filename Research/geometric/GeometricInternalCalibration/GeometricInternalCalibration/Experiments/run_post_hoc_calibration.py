...
# Import data loaders  
from Data.cifar10 import get_train_valid_loader as cifar10_loaders, get_test_loader as cifar10_test
from Data.cifar100 import get_train_valid_loader as cifar100_loaders, get_test_loader as cifar100_test
from Data.svhn import get_train_valid_loader as svhn_loaders, get_test_loader as svhn_test
from Data.pacs import get_train_valid_loader as pacs_loaders, get_test_loader as pacs_test
from Data.tiny_imagenet import get_data_loader as tiny_imagenet_get_data_loader, TinyImageNet
...
class PyTorchModelAdapter:
    ...
    def __init__(self, pytorch_model, device, dataset_name='cifar10', feature_extractor=None):
        ...
        if feature_extractor:
            logger.info(f"✅ Feature extractor integrated for {feature_extractor.model_name}")
        if self.requires_224_input:
            logger.info(f"⚠️  Model requires 224x224 input - will resize images automatically")
    ...
    def _detect_224_input_requirement(self) -> bool:
        ...
        if 'dinov2' in model_str or 'vision_transformer' in model_str or 'vit' in model_str:
            return True
    ...
    def predict_proba(self, X, batch_size: int = 256):
        ...
        if X_t.ndim == 2 and X_t.shape[1] in [10, 100]:
            raise ValueError(f"predict_proba received logits-shaped tensor: {tuple(X_t.shape)}")
        ...
        for (xb,) in dl:
            ...
            if len(probs_chunks) == 0:
                try:
                    ...
                    if std_v < 1e-4:
                        logger.error("❌ CRITICAL: Logits have near-zero variance! This will produce uniform probabilities.")
                        logger.error(f"   This suggests the model is not making predictions (weights might not be loaded).")

                    # Check if logits are all zeros or very small
                    if abs(mean_v) < 1e-6 and std_v < 1e-6:
                        logger.error("❌ CRITICAL: Logits are all near-zero! Model appears uninitialized.")
    ...
    def predict_logits(self, X, batch_size: int = 256):
        ...
        try:
            ...
            logger.info(f"Logits shape: {tuple(logits_tensor.shape)} | range: [{min_v:.4f}, {max_v:.4f}] | mean: {mean_v:.4f} | std: {std_v:.4f}")
        except Exception:
            pass
        ...
        try:
            ...
            if looks_prob:
                ...
                if torch.allclose(row_sums, torch.ones_like(row_sums), atol=1e-3, rtol=1e-3):
                    logger.warning("Values look like probabilities, not logits. Converting to log-probabilities for downstream calibration.")
...
class FeatureExtractor:
    ...
    def _register_hook(self):
        ...
        if self.model_name in extraction_config:
            ...
            logger.warning(f"⚠️ Using fallback extraction for {self.model_name}")
    ...
    def _get_target_layer(self, layer_name: str):
        ...
        if 'resnet' in self.model_name:
            ...
            if hasattr(self.model, 'block3'):
                ...
                raise ValueError("Wide-ResNet target layer not found (expected block3/layer3)")
            ...
            if layer_name == 'denseblock3':
                ...
                # Final fallback
                logger.warning("⚠️ Could not find denseblock3, using features module")
        ...
        if self.model_name in dimension_ranges:
            ...
            def validation_hook(module, input, output):
                ...
                if feat_dim < min_dim or feat_dim > max_dim:
                    logger.warning(f"⚠️ Feature dimension {feat_dim} outside expected range {min_dim}-{max_dim}")
                else:
                    logger.info(f"✅ Feature dimension {feat_dim} within expected range")
...
def load_trained_model(model_path: str, model_name: str, num_classes: int, device, dataset: str = 'cifar10'):
    ...
    if dataset == 'pacs':
        ...
        if model_name == 'resnet18':
        ...
        elif any(token in model_name.lower() for token in ('wide-resnet', 'wide_resnet', 'wideresnet')):
            ...
            if (depth - 4) % 6 == 0:
                ...
                raise ValueError(f"Unsupported Wide-ResNet depth: {depth}. Expected 6n+4 or 6n+2.")
            ...
            if os.path.exists(model_path):
                ...
                if isinstance(checkpoint_state, dict):
                    ...
                    if 'temp' in actual_state:
                        ...
                        logger.info(f"🌡️ Extracted temperature from checkpoint: {temp_param}")

            model = Wide_ResNet_Cifar(BasicBlock, layers, widen_factor, num_classes=num_classes, temp=temp_param)
            logger.info(f"🔧 Wide-ResNet instantiated: depth={depth}, width={widen_factor}, temp={temp_param}")
            logger.info(f"   Model type: {type(model)}")
            logger.info(f"   Has base_model attr: {hasattr(model, 'base_model')}")
            logger.info(f"   Model has fc layer: {hasattr(model, 'fc')}")
            if hasattr(model, 'fc'):
                logger.info(f"   FC layer shape: in={model.fc.in_features}, out={model.fc.out_features}")
            ...
            is_timm_style = any(k.startswith('backbone.blocks.') or k == 'backbone.cls_token' for k in checkpoint_keys)
            
            if is_timm_style:
                logger.info("🔍 Detected timm-style DINOv2 checkpoint, using timm model")
                import timm
                ...
                backbone = timm.create_model(timm_name, pretrained=False, num_classes=0, img_size=224)
                ...
                logger.info(f"✅ Created timm DINOv2 model: {timm_name} with {num_classes} classes")
                ...
                logger.info("🔍 Detected HuggingFace-style DINOv2 checkpoint, using HuggingFace model")
    ...
    if isinstance(state_dict, dict) and "model_state" in state_dict:
        logger.info("🔍 Detected wrapped checkpoint with 'model_state'; extracting weights")
        state_dict = state_dict["model_state"]
    
    model_keys = list(model.state_dict().keys())
    checkpoint_keys = list(state_dict.keys())
    
    logger.info(f"📊 Checkpoint has {len(checkpoint_keys)} keys, model expects {len(model_keys)} keys")
    ...
    def try_load_with_remapping(candidate_state, strategy_name):
        ...
        try:
            ...
            logger.info(f"Strategy '{strategy_name}': loaded {loaded}/{len(candidate_state)} keys")
    ...
    best_loaded, best_missing, best_unexpected = try_load_with_remapping(best_state_dict, best_strategy)

    # Strategy 2: Remove 'module.' prefix
    if any(k.startswith('module.') for k in state_dict.keys()):
        cleaned = remap_keys(state_dict, lambda k: k.replace('module.', '', 1) if k.startswith('module.') else k)
    ...
    if any(k.startswith('base_model.') for k in state_dict.keys()):
        cleaned = remap_keys(state_dict, lambda k: k.replace('base_model.', '', 1) if k.startswith('base_model.') else k)
        loaded, missing, unexpected = try_load_with_remapping(cleaned, "remove_base_model")
        if loaded > best_loaded:
            best_loaded, best_missing, best_unexpected = loaded, missing, unexpected
            best_state_dict = cleaned
    ...
    if 'resnet' in model_name.lower() and any('downsample' in k for k in best_state_dict.keys()):
        cleaned = remap_keys(best_state_dict, lambda k: k.replace('downsample', 'shortcut'))
        loaded, missing, unexpected = try_load_with_remapping(cleaned, "downsample_to_shortcut")
        if loaded > best_loaded:
            best_loaded, best_missing, best_unexpected = loaded, missing, unexpected
            best_state_dict = cleaned
    ...
    if 'wide' in model_name.lower() and 'resnet' in model_name.lower():
        if any('shortcut' in k for k in best_state_dict.keys()):
            cleaned = remap_keys(best_state_dict, lambda k: k.replace('shortcut', 'downsample'))
            loaded, missing, unexpected = try_load_with_remapping(cleaned, "shortcut_to_downsample")
            if loaded > best_loaded:
                best_loaded, best_missing, best_unexpected = loaded, missing, unexpected
                best_state_dict = cleaned
    ...
    if 'dinov2' in model_name.lower():
        ...
        if is_timm_model:
            ...
            if any(k.startswith('backbone.blocks.') or k == 'backbone.cls_token' for k in best_state_dict.keys()):
                # Try direct loading first (keys should match)
                loaded, missing, unexpected = try_load_with_remapping(best_state_dict, "dinov2_timm_direct")
                if loaded > best_loaded:
                    best_loaded, best_missing, best_unexpected = loaded, missing, unexpected
            ...
            def remap_dinov2_keys(key):
                ...
                if block_match:
                    ...
                    # Layer scale
                    if rest == 'ls1.gamma':
                        return f'backbone.encoder.layer.{block_num}.layer_scale1.lambda1'
                    if rest == 'ls2.gamma':
                        return f'backbone.encoder.layer.{block_num}.layer_scale2.lambda1'
            ...
            if any(k.startswith('backbone.blocks.') or k == 'backbone.cls_token' for k in best_state_dict.keys()):
                ...
                for k, v in best_state_dict.items():
                    ...
                    if new_key is None:
                        ...
                        block_match = re.match(r'backbone\.blocks\.(\d+)\.attn\.qkv\.(.+)', k)
                        if block_match:
                            block_num = block_match.group(1)
                ...
                for (block_num, suffix), qkv in qkv_weights.items():
                    ...
                    remapped[f'backbone.encoder.layer.{block_num}.attention.attention.query.{suffix}'] = q
                    remapped[f'backbone.encoder.layer.{block_num}.attention.attention.key.{suffix}'] = k
                    remapped[f'backbone.encoder.layer.{block_num}.attention.attention.value.{suffix}'] = v
                
                loaded, missing, unexpected = try_load_with_remapping(remapped, "dinov2_timm_to_hf")
                if loaded > best_loaded:
                    best_loaded, best_missing, best_unexpected = loaded, missing, unexpected
    ...
    final_missing, final_unexpected = model.load_state_dict(best_state_dict, strict=False)
    ...
    if final_missing:
        logger.info(f"   Remaining missing keys after best strategy: {final_missing[:5]}")
    if final_unexpected:
        logger.info(f"   Remaining unexpected keys after best strategy: {final_unexpected[:5]}")
    ...
    if 'wide' in model_name.lower() and 'resnet' in model_name.lower():
        ...
        if hasattr(model, 'fc') and isinstance(model.fc, nn.Linear):
            ...
            logger.info(f"   FC weight stats: mean={fc_weight_mean:.6f}, std={fc_weight_std:.6f}, abs_max={fc_weight_abs_max:.6f}")

            if fc_bias is not None:
                fc_bias_mean = float(fc_bias.mean().item())
                fc_bias_std = float(fc_bias.std().item())
                logger.info(f"   FC bias stats: mean={fc_bias_mean:.6f}, std={fc_bias_std:.6f}")

            # Check if weights look uninitialized (all zeros or very small values)
            if fc_weight_abs_max < 1e-6:
                logger.error("❌ FC layer weights appear to be zeros! Model may not be properly loaded.")
            elif fc_weight_std < 1e-6:
                logger.warning("⚠️ FC layer weights have zero variance - this is suspicious!")
            else:
        ...
        if hasattr(model, 'conv1'):
            ...
            logger.info(f"   Conv1 weight stats: mean={conv1_mean:.6f}, std={conv1_std:.6f}")

    model = model.to(device)
    logger.info(f"✅ Model loaded successfully with {matched_keys}/{len(model_keys)} model keys")
    return model
...
def get_data_loaders(
    ...
    def wrap_transform_with_resize(base_transform, image_size):
        ...
        if totensor_idx >= 0:
            transform_list.insert(totensor_idx, transforms.Resize((image_size, image_size)))
        else:
    ...
    if dataset == 'cifar10':
        ...
        if image_size is not None and image_size != 32:
            ...
            for dataset in [train_loader.dataset, val_loader.dataset]:
                if hasattr(dataset, 'transform') and dataset.transform is not None:
                    dataset.transform = wrap_transform_with_resize(dataset.transform, image_size)
        ...
        if corruption_type is not None and corruption_severity is not None:
            ...
            if image_size is not None and image_size != 32 and hasattr(test_loader.dataset, 'transform'):
                test_loader.dataset.transform = wrap_transform_with_resize(
                    test_loader.dataset.transform or transforms.ToTensor(), 
                    image_size
                )
        else:
            ...
            if image_size is not None and image_size != 32 and hasattr(test_loader.dataset, 'transform'):
                test_loader.dataset.transform = wrap_transform_with_resize(test_loader.dataset.transform, image_size)
...
def construct_model_path(base_dir: str, method: str, dataset: str, model: str, seed: int, target_domain: str = 'sketch') -> str:
    ...
    if is_dinov2 and not is_baseline_method:
        ...
        calibration_pattern = os.path.join(
            base_dir, "calibration_comparison", 
        ...
        if matching_files:
            ...
            logger.info(f"🗂️ Found DINOv2 model in calibration_comparison: {model_path}")
            return model_path
        else:
            logger.warning(f"⚠️ DINOv2 model not found with pattern: {calibration_pattern}, trying standard path")
    ...
    if os.path.exists(flat_checkpoint):
        logger.info(f"🗂️ Using flat checkpoint path: {flat_checkpoint}")
        return flat_checkpoint

    logger.info(f"🗂️ Flat checkpoint missing; falling back to nested path: {nested_checkpoint}")
...
def extract_raw_data_and_features(feature_extractor, data_loader, device):
        ...
        for batch_idx, (data, targets) in enumerate(data_loader):
            try:
                ...
                if data.numel() == 0 or targets.numel() == 0:
                    logger.warning(f"⚠️ Batch {batch_idx}: Empty input data, skipping")
                    failed_batches += 1
                    continue
                
                # Check for NaN or infinite values in input
                if torch.isnan(data).any() or torch.isinf(data).any():
                    logger.warning(f"⚠️ Batch {batch_idx}: Invalid input data (NaN/Inf), skipping")
                    failed_batches += 1
                    continue
                ...
                if logits is None or logits.numel() == 0:
                    logger.warning(f"⚠️ Batch {batch_idx}: Empty logits output, skipping")
                    failed_batches += 1
                    continue
                
                # Check for NaN or infinite values in logits
                if torch.isnan(logits).any() or torch.isinf(logits).any():
                    logger.warning(f"⚠️ Batch {batch_idx}: Invalid logits (NaN/Inf), skipping")
                    failed_batches += 1
                    continue
                ...
                if feature_extractor.features is not None:
                    ...
                    if len(features.shape) == 4:  # [B, C, H, W]
                        features = features.mean(dim=[2, 3])  # Global average pooling
                elif hasattr(feature_extractor, 'layer_features') and feature_extractor.layer_features:
                    # Try to get features from layer_features dict
                    if feature_extractor.forced_layer and feature_extractor.forced_layer in feature_extractor.layer_features:
                        features = feature_extractor.layer_features[feature_extractor.forced_layer]
                ...
                if features is None:
                    logger.warning(f"⚠️ Batch {batch_idx}: No features extracted, skipping")
                    failed_batches += 1
                    continue
                
                if features.numel() == 0:
                    logger.warning(f"⚠️ Batch {batch_idx}: Empty features extracted, skipping")
                    failed_batches += 1
                    continue
                
                # Check for NaN or infinite values in features
                if torch.isnan(features).any() or torch.isinf(features).any():
                    logger.warning(f"⚠️ Batch {batch_idx}: Invalid features (NaN/Inf), skipping")
                    failed_batches += 1
                    continue
                ...
                if batch_idx == 0:
                    ...
                    logger.info(f"Raw data stats: mean={data.mean().item():.6f}, std={data.std().item():.6f}")
                    logger.info(f"Features shape: {features.shape}")
                    logger.info(f"Features stats: mean={features.mean().item():.6f}, std={features.std().item():.6f}")
                    logger.info(f"Logits shape: {logits.shape}")
                    logger.info(f"Logits stats: mean={logits.mean().item():.6f}, std={logits.std().item():.6f}")

                    # Check if features look like logits (this would be a bug)
                    if features.shape[1] == 10 or features.shape[1] == 100:
                        logger.warning("⚠️ WARNING: Features have 10/100 dims - might be logits instead of features!")

                    # Check if features are diverse (not uniform)
                    feature_variance = torch.var(features, dim=0).mean().item()
                    if feature_variance < 0.001:
                        logger.warning("⚠️ WARNING: Features have very low variance - extraction might have failed!")
    ...
    logger.info(f"📊 Extraction summary: {successful_batches} successful, {failed_batches} failed batches")
    ...
    def validate_tensor_list(tensor_list, name):
        ...
        for i, tensor in enumerate(tensor_list):
            if tensor is None:
                logger.warning(f"⚠️ None tensor in {name} at index {i}, skipping")
                continue
            if tensor.numel() == 0:
                logger.warning(f"⚠️ Empty tensor in {name} at index {i}, skipping")
                continue
            if torch.isnan(tensor).any() or torch.isinf(tensor).any():
                logger.warning(f"⚠️ Invalid tensor in {name} at index {i}, skipping")
                continue
    ...
    try:
        ...
        min_length = min(len(all_raw_data), len(all_features), len(all_logits), len(all_labels))
        if min_length < len(all_raw_data):
            logger.warning(f"⚠️ Truncating tensor lists to consistent length: {min_length}")
    ...
    try:
        ...
        # Print debug info
        logger.error(f"Tensor list lengths: raw_data={len(all_raw_data)}, features={len(all_features)}, logits={len(all_logits)}, labels={len(all_labels)}")
        if all_features:
            logger.error(f"Feature shapes: {[f.shape for f in all_features[:5]]}")  # First 5 shapes
        raise


def evaluate_on_corrupted_test(calibrators: dict, dataset_name: str, model_adapter, feature_extractor, 
                              device, cifar10c_dir: str = None, cifar100c_dir: str = None):
    """
    ...
    if dataset_name == 'cifar10':
        if cifar10c_dir is None:
            logger.warning("⚠️ CIFAR-10-C directory not provided, skipping corruption evaluation")
            return {}
        corruption_data_dir = cifar10c_dir
        loader_fn = get_cifar10c_loader
    elif dataset_name == 'cifar100':
        if cifar100c_dir is None:
            logger.warning("⚠️ CIFAR-100-C directory not provided, skipping corruption evaluation")
            return {}
        corruption_data_dir = cifar100c_dir
        loader_fn = get_cifar100c_loader
    else:
        logger.warning(f"⚠️ Corruption evaluation not supported for {dataset_name}")
        return {}
    
    # Check if corruption data directory exists
    if not os.path.exists(corruption_data_dir):
        logger.warning(f"⚠️ Corruption data directory not found: {corruption_data_dir}")
        return {}
    ...
    for cal_name, calibrator in calibrators.items():
            ...
            for severity in range(1, 6):
                try:
                    ...
                    try:
                        ...
                        logger.error(f"      ❌ Failed {corruption} severity {severity}: {extract_error}")
                    ...
                    try:
                        if cal_name == 'Uncalibrated':
                            cal_probs = F.softmax(torch.tensor(test_logits), dim=1).numpy()
                        elif cal_name.startswith('Geometric'):
                            # For geometric methods, decide input type
                            if 'Semantic' in cal_name:
                                cal_probs = calibrator.calibrate(test_features, X_test_original=test_raw)
                            else:
                                cal_probs = calibrator.calibrate(test_raw, X_test_original=test_raw)
                        else:
                        ...
                        ece = expected_calibration_error(cal_confs, cal_preds, test_labels)
                        ...
                        logger.error(f"      ❌ Calibration failed for {corruption} severity {severity}: {calib_error}")
        ...
        logger.info(f"      Evaluations: {successful_evaluations} successful, {failed_evaluations} failed")
...
def calculate_mce_for_all_calibrators(corruption_results: dict):
    ...
    if 'Uncalibrated' not in corruption_results:
        logger.warning("⚠️ Cannot calculate mCE without uncalibrated baseline")
...
def combine_clean_and_corruption_results(clean_results: dict, corruption_results: dict, mce_results: dict):
    """
    ...
    for cal_name in clean_results.keys():
        enhanced_results[cal_name] = {
            ...
            'corruption_accuracy': corruption_results.get(cal_name, {}).get('mean_corruption_accuracy', 0.0),
            'corruption_ece': corruption_results.get(cal_name, {}).get('mean_corruption_ece', 1.0),
            'mce': mce_results.get(cal_name, None),
            'corruption_error_rate': 100.0 - corruption_results.get(cal_name, {}).get('mean_corruption_accuracy', 0.0)
        }
    
    return enhanced_results


def evaluate_calibration_methods_with_corruption(model, feature_extractor, train_loader, val_loader, test_loader, 
                                               device, num_classes, method_name, dataset_name, 
                                               cifar10c_dir=None, cifar100c_dir=None):
    """Enhanced evaluation with corruption robustness and proper data handling"""
    
    logger.info(f"🔬 Starting ENHANCED calibration + corruption evaluation for {method_name}...")
    ...
    for cal_name, calibrator in calibrators.items():
        ...
        try:
            if cal_name == 'Uncalibrated':
                ...
                start_time = time.time()
                if 'Semantic' in cal_name:
                    test_probs = calibrator.calibrate(test_features, X_test_original=test_raw)
                else:
                    test_probs = calibrator.calibrate(test_raw, X_test_original=test_raw)
                calibration_time = time.time() - start_time
            ...
            logger.info(f"   {cal_name}: Accuracy={accuracy:.2f}%, ECE={ece:.4f}, Time={calibration_time:.2f}s")
    ...
    logger.info(f"🌪️ Starting corruption robustness evaluation...")
    corruption_results = evaluate_on_corrupted_test(
        calibrators, dataset_name, model_adapter, feature_extractor, device,
        cifar10c_dir=cifar10c_dir, cifar100c_dir=cifar100c_dir
    )
    ...
    enhanced_results = combine_clean_and_corruption_results(clean_results, corruption_results, mce_results)
    
    return enhanced_results


def run_calibration_experiment_fixed(model_adapter, train_features, train_raw, train_labels,
                                   val_features, val_raw, val_labels,
                                   test_features, test_raw, test_labels, test_logits,
                                   dataset_name):
    """
        ...
        try:
            ...
            if config['type'] == 'baseline':
                ...
                if use_semantic:
                    ...
                    logger.info("🖼️ Using raw images for geometric calculations")
...
def create_enhanced_results_table(results_dict: dict, dataset: str, model_name: str) -> str:
    ...
    for method, results in results_dict.items():
        for cal_name, cal_results in results.items():
            if 'error' in cal_results:
                ...
                row = [cal_name, clean_acc, clean_ece, corruption_acc, mce_str, calibration_time]
            
            table_data.append(row)
    
    # Create the comprehensive table
    headers = ['Method', 'Clean Acc', 'Clean ECE', 'Corruption Acc', 'mCE', 'Time']
    comprehensive_table = tabulate(table_data, headers=headers, tablefmt='grid', floatfmt='.4f')
    
    # Find best performers
    valid_results = {cal_name: cal_results for method, results in results_dict.items() 
                    for cal_name, cal_results in results.items() if 'error' not in cal_results}
    ...
    if valid_results:
        ...
        valid_mce_results = {name: results for name, results in valid_results.items() 
        ...
        if valid_mce_results:
            ...
            best_performers = f"\n🏆 Best Calibration (Clean): {best_cal_name} (ECE={best_cal_results['ece']:.4f})"
            best_performers += f"\n⚠️ Corruption results not available for robustness comparison"
    else:
...
def create_results_table(results_dict: dict, dataset: str, model_name: str) -> str:
    ...
    # Prepare table data
    table_data = []
    headers = ['Method', 'Uncalibrated', 'Temperature', 'Isotonic', 
              'Geometric_Semantic_FAISS', 'Geometric_Physical_FAISS',
              'Geometric_Semantic_Fast_Separation', 'Geometric_Physical_Fast_Separation',
              # NEW: AugMix-enhanced calibrators
              'Geometric_Semantic_FAISS_AugMix', 'Geometric_Physical_FAISS_AugMix',
              'Geometric_Semantic_Fast_Separation_AugMix', 'Geometric_Physical_Fast_Separation_AugMix']
    ...
    for method, results in results_dict.items():
            ...
            if cal_method in results:
                if 'error' in results[cal_method]:
                    ...
                    if 'calibration_time' in results[cal_method]:
                        timing_row.append(f"{results[cal_method]['calibration_time']:.2f}s")
                    else:
        ...
        for cal_method in ['Geometric_Semantic_FAISS', 'Geometric_Physical_FAISS',
                          'Geometric_Semantic_Fast_Separation', 'Geometric_Physical_Fast_Separation',
                          # NEW: AugMix-enhanced calibrators
                          'Geometric_Semantic_FAISS_AugMix', 'Geometric_Physical_FAISS_AugMix',
                          'Geometric_Semantic_Fast_Separation_AugMix', 'Geometric_Physical_Fast_Separation_AugMix']:
    ...
    ece_table = tabulate(ece_table_data, headers=headers, tablefmt='grid', floatfmt='.4f')
    timing_table = tabulate(timing_table_data, headers=headers, tablefmt='grid', floatfmt='.2f')
...
def check_results_completeness(output_file: str) -> bool:
    ...
    expected_calibrators = [
        'Uncalibrated', 'Temperature', 'Isotonic', 
        'Geometric_Semantic_FAISS', 'Geometric_Physical_FAISS',
        'Geometric_Semantic_Fast_Separation', 'Geometric_Physical_Fast_Separation',
        # NEW: AugMix-enhanced calibrators
        'Geometric_Semantic_FAISS_AugMix', 'Geometric_Physical_FAISS_AugMix',
    ...
    try:
        ...
        for calibrator in expected_calibrators:
            ...
            missing_fields = [field for field in required_fields if field not in calibrator_result]
            if missing_fields:
...
def main():
    ...
    parser.add_argument('--output_dir', type=str,
                       default='aaai_full_experiments/phase2_calibration',
    ...
    # Construct paths
    model_path = construct_model_path(args.results_dir, args.method, args.dataset, args.model, args.seed)
    ...
    if accuracy < 50.0:
        logger.info("⚠️ Model accuracy is below 50%. Exiting peacefully.")
        return
    ...
    if os.path.exists(output_file):
        try:
            with open(output_file, 'r') as f:
                ...
                logger.info(f"📄 Loaded existing results with {len(existing_results)} calibrators")
        except Exception as e:
    ...
    if not missing_calibrators and not args.enable_corruption_eval:
        logger.info("✅ All expected calibrators are present in the results. No need to run additional calculations.")
        return

    logger.info(f"🔍 Found {len(missing_calibrators)} missing calibrators: {missing_calibrators}")
    ...
    try:
        ...
        if args.enable_corruption_eval:
            logger.info("🌪️ ENHANCED EVALUATION MODE: Including corruption robustness assessment")
            ...
            enhanced_results = evaluate_calibration_methods_with_corruption(
                ...
                device, num_classes, args.method, args.dataset,
                cifar10c_dir=args.cifar10c_dir, cifar100c_dir=args.cifar100c_dir
            )
            
            # Create enhanced results table and display
            enhanced_results_dict = {args.method: enhanced_results}
            enhanced_table_str = create_enhanced_results_table(enhanced_results_dict, args.dataset, args.model)
        ...
        model_adapter = PyTorchModelAdapter(model, device, args.dataset, feature_extractor)
        
        # Initialize results with existing data
        results = existing_results.copy()
        
        # 🔧 FIXED: Use the corrected calibration experiment function
        logger.info("🔧 Using fixed calibration experiment with proper data type handling")
