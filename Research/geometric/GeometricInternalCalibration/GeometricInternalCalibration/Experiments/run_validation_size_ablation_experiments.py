#!/usr/bin/env python3
"""
run_validation_size_ablation_experiments.py

Parallel runner for the validation size ablation experiment (Experiments/run_validation_size_ablation.py)
using SLURM. Submits jobs for different (model, dataset, seed) combinations.

Usage:
    python run_validation_size_ablation_experiments.py --config configs/validation_size_ablation_sweep.yaml

Or with inline arguments:
    python run_validation_size_ablation_experiments.py \
        --models resnet18 densenet121 \
        --datasets cifar10 cifar100 \
        --seeds 11 12 13 \
        --results-base-dir /path/to/models \
        --output-base-dir results/validation_size_ablation
"""
...
def generate_sbatch_command(
    ...
    num_trials: int = 3,
    target_dim: int = 256,
    layer_mode: str = 'random_6',
    conda_env: str = "geo_cuda12",
) -> str:
    """
    Generate an sbatch command string for a single validation size ablation job.
    ...
    log_dir = Path(output_dir) / "_validation_size_ablation_logs"
    ...
    python_cmd = [
        sys.executable,
        str(project_root / "Experiments" / "run_validation_size_ablation.py"),
        ...
        "--val_sizes", *[str(vs) for vs in val_sizes],
    ...
    wrap_script = f"""
echo '========================================'
echo '🔬 SLURM JOB: Validation Size Ablation Study'
echo '========================================'
...
echo '  Val Sizes  : {" ".join(str(vs) for vs in val_sizes)}'
echo '  Num Trials : {num_trials}'
echo '  Target Dim : {target_dim}'
echo '  Layer Mode : {layer_mode}'
echo '----------------------------------------'
...
def check_metrics_in_results(result_file: str) -> Tuple[bool, str]:
    """
    Check if a completed results file contains the summary section produced by run_validation_size_ablation.py.

    Args:
        result_file: Path to the validation size ablation results JSON file
...
def filter_existing_experiments(
    ...
    Args:
        ...
        output_base_dir: Base output directory for validation size ablation results
    ...
    for exp in experiments:
        result_file = os.path.join(
            output_base_dir,
            f"{exp['model_name']}_{exp['dataset_name']}_seed{exp['seed']}_validation_size_ablation.json"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Parallel runner for validation size ablation experiment",
    ...
    parser.add_argument('--val-sizes', nargs='+', type=int, default=[50, 100, 200, 500, 1000, 2000, 5000],
                        dest='val_sizes',
                        help="Validation sizes to evaluate in each job")
    ...
    parser.add_argument('--layer-mode', type=str, default='random_6',
                        dest='layer_mode',
    ...
    parser.add_argument('--output-base-dir', type=str, default=None,
                        help="Output directory for validation size ablation results")
    ...
    for seed in sorted(experiments_by_seed.keys()):
        ...
        for model in sorted(experiments_by_seed[seed].keys()):
            ...
            for exp in sorted_exps:
                job_name = f"valabl_{exp['model_name']}_{exp['dataset_name']}_{exp['training_method']}_s{exp['seed']}"
    ...
    if args.dry_run:
        ...
        logger.info(f"📁 SLURM logs in: {args.slurm_output_dir}/_validation_size_ablation_logs/")
