...
def generate_sbatch_command(
    ...
    compute_brier_top_label: bool = False,
    ...
    gpu_type = slurm_config.get('gpu_type', '')
    ...
    if deep_ensemble_scan:
        python_cmd.extend([
            ...
            "--seed", str(seed),  # Used as master_seed for random group sampling
        ])
    else:
        ...
        python_cmd.extend([
            ...
            "--target-dimension", str(target_dimension),
        ])
        if corruption_type:
        ...
        if force_recompute:
            python_cmd.append("--force-recompute")
        ...
        if baselines_only:
            python_cmd.append("--baselines-only")
        if compute_brier_top_label:
            python_cmd.append("--compute-brier-top-label")
    if corruption_type and "--corruption-type" not in python_cmd:
        python_cmd.extend(["--corruption-type", corruption_type])
    if corruption_severity is not None and "--corruption-severity" not in python_cmd:
        python_cmd.extend(["--corruption-severity", str(corruption_severity)])
    if skip_single_layers and "--skip-single-layers" not in python_cmd:
        python_cmd.append("--skip-single-layers")
    if force_recompute and "--force-recompute" not in python_cmd:
        python_cmd.append("--force-recompute")
    if force_rerun_geometric and "--force-rerun-geometric" not in python_cmd:
        python_cmd.append("--force-rerun-geometric")
    if baselines_only and "--baselines-only" not in python_cmd:
        python_cmd.append("--baselines-only")
    if compute_brier_top_label and "--compute-brier-top-label" not in python_cmd:
        python_cmd.append("--compute-brier-top-label")
    if methods:
    ...
    if force_rerun_geometric and "--force-rerun-geometric" not in safe_python_cmd:
        logger.warning(f"WARNING: --force-rerun-geometric flag was set but not found in command for {job_name}")
    ...
    baselines_only_str = "True" if baselines_only else "False"
...
elif [ "{baselines_only_str}" = "True" ]; then
    echo '  Mode       : Baselines Only'
    echo '  Top-label Brier : {"Enabled" if compute_brier_top_label else "Disabled"}'
else
...
"""
    ...
    # Build sbatch command. Only request GPUs for GPU jobs; CPU partitions often
    # reject --gpus=0 or --gpus=:0.
    sbatch_cmd = [
        "sbatch",
    ...
    if int(gpus) > 0:
        gpu_request = f"{gpu_type}:{gpus}" if gpu_type else str(gpus)
        sbatch_cmd.insert(6, f"--gpus={shlex.quote(gpu_request)}")
...
def validate_experiment_model(
    ...
    if not os.path.exists(results_path):
        try:
            ...
            return False, f"Results file not found: {results_path}; error constructing model path: {e}"

        if os.path.exists(model_path):
            return True, (
                f"Model checkpoint found at {model_path}; "
                f"training results.json missing, skipping accuracy precheck"
            )
...
def check_metrics_in_results(result_file: str) -> Tuple[bool, str]:
    ...
    try:
            ...
            if isinstance(geo_original, dict) and geo_original.get('skipped') is True:
                ...
                for layer_key, layer_result in geo_original.items():
                    if not is_result_complete(layer_result, GEO_REQUIRED_FIELDS):
                        return False, f"Incomplete 'geometric_original.{layer_key}' section (missing calibrator_params)"
            else:
        ...
        if 'geometric_dac_weighted' in results:
            if not is_section_complete(results, 'geometric_dac_weighted', DAC_REQUIRED_FIELDS):
                return False, "Incomplete 'geometric_dac_weighted' section (missing layer_weights)"
        ...
        if 'ablation' in results:
            ...
            if 'dac_with_sgc_aggregated_features' in ablation:
                if not is_section_complete(results, 'ablation.dac_with_sgc_aggregated_features', DAC_REQUIRED_FIELDS):
                    return False, "Incomplete 'ablation.dac_with_sgc_aggregated_features' (missing layer_weights)"
            if 'dac_with_random_layer_selection' in ablation:
                if not is_section_complete(results, 'ablation.dac_with_random_layer_selection', DAC_REQUIRED_FIELDS):
                    return False, "Incomplete 'ablation.dac_with_random_layer_selection' (missing layer_weights)"
            # Geometric-based ablations need calibrator_params
            if 'sgc_with_dac_preprocessing' in ablation:
                if not is_section_complete(results, 'ablation.sgc_with_dac_preprocessing', GEO_REQUIRED_FIELDS):
                    return False, "Incomplete 'ablation.sgc_with_dac_preprocessing' (missing calibrator_params)"
            if 'random_single_layer_dac_preprocessing' in ablation:
                if not is_section_complete(results, 'ablation.random_single_layer_dac_preprocessing', GEO_REQUIRED_FIELDS):
                    return False, "Incomplete 'ablation.random_single_layer_dac_preprocessing' (missing calibrator_params)"
        
        # Check other geometric sections
        geometric_sections = ['global_random', 'coordinate_sampling', 'coordinate_spp', 'coordinate_spp_dac', 
                              'geometric_concatenated', 'sgc_faiss', 'coordinate_sampling_faiss', 'coordinate_spp_faiss']
        for section in geometric_sections:
            if section in results:
                if not is_section_complete(results, section, GEO_REQUIRED_FIELDS):
                    return False, f"Incomplete '{section}' section (missing calibrator_params)"
        
        # Check DAC sections
        if 'dac_with_coordinate_features' in results:
            if not is_section_complete(results, 'dac_with_coordinate_features', DAC_REQUIRED_FIELDS):
                return False, "Incomplete 'dac_with_coordinate_features' section (missing layer_weights)"
            ...
            if isinstance(norm_ablation, dict) and 'skipped' not in norm_ablation:
                ...
                for exp_name, exp_result in norm_ablation.items():
                    if not is_result_complete(exp_result, GEO_REQUIRED_FIELDS):
                        return False, f"Incomplete 'normalization_ablation.{exp_name}' (missing calibrator_params)"
            ...
            if isinstance(metric_data, dict) and metric_data.get('skipped') is True:
                ...
                for entry in metric_data:
                    if not is_result_complete(entry, GEO_REQUIRED_FIELDS):
                        return False, "'metric_guided_calibration' has incomplete entries (missing calibrator_params)"
            elif isinstance(metric_data, dict):
                if len(metric_data) == 0:
                    return False, "'metric_guided_calibration' dict is empty"
        
        # Check analysis section (not required for deep ensemble scan)
        if 'analysis' not in results or 'best_overall_method' not in results.get('analysis', {}):
            ...
            if 'deep_ensemble_scan' in results:
                ...
                if isinstance(ensemble_data, dict):
                    ...
                    required_ensemble_fields = ['ece_mean', 'ece_std', 'accuracy_mean', 'n_groups']
                    if all(k in ensemble_data for k in required_ensemble_fields):
                        return True, "Deep ensemble scan complete"
                    else:
                        return False, f"Deep ensemble scan incomplete (missing: {[k for k in required_ensemble_fields if k not in ensemble_data]})"
...
def check_baseline_metrics_in_results(
    result_file: str,
    require_top_label_brier: bool = False,
) -> Tuple[bool, str]:
    """Check whether standard baseline results have all fields needed by paper tables."""
    import json
    ...
    try:
        ...
        required_fields = ['ece', 'adaptive_ece', 'calibration_mce']
        if require_top_label_brier:
            required_fields.append('top_label_brier')
        ...
        for method in required_methods:
            ...
            if not isinstance(entry, dict):
                return False, f"Missing baseline '{method}'"
            if entry.get('error'):
                return False, f"Baseline '{method}' has error: {entry.get('error')}"
            missing = [field for field in required_fields if entry.get(field) is None]
            if missing:
                return False, f"Baseline '{method}' missing {missing}"

        dac_entry = baselines.get('density_aware_calibration')
        if isinstance(dac_entry, dict) and not dac_entry.get('skipped', False):
            if dac_entry.get('error'):
                return False, f"DAC baseline has error: {dac_entry.get('error')}"
            missing = [field for field in required_fields if dac_entry.get(field) is None]
            if missing:
                return False, f"DAC baseline missing {missing}"

        geometric_entry = baselines.get('geometric_physical_space')
        if not isinstance(geometric_entry, dict):
            return False, "Missing baseline 'geometric_physical_space'"
        if geometric_entry.get('error'):
            return False, f"Geometric physical-space baseline has error: {geometric_entry.get('error')}"
        missing = [field for field in required_fields if geometric_entry.get(field) is None]
        if missing:
            return False, f"Geometric physical-space baseline missing {missing}"

        raw_geometric_entry = baselines.get('geometric_raw_images')
        if isinstance(raw_geometric_entry, dict) and not raw_geometric_entry.get('skipped', False):
            if raw_geometric_entry.get('error'):
                pass
            else:
                missing = [field for field in required_fields if raw_geometric_entry.get(field) is None]
                if missing:
                    return False, f"Geometric raw-images baseline missing {missing}"

        metric_label = "ECE, Adaptive ECE, MCE"
        if require_top_label_brier:
            metric_label += ", and top-label Brier"
        return True, f"Standard baselines complete with {metric_label}"
...
def filter_existing_experiments(
    ...
    for exp in experiments:
        ...
        if skip_completed and os.path.exists(result_file):
            ...
            if baselines_only:
                is_complete, reason = check_baseline_metrics_in_results(
                    result_file,
                    require_top_label_brier=compute_brier_top_label,
                )
            else:
...
def main():
    ...
    parser.add_argument('--corruption-severity-levels', nargs='+', type=int, default=None,
    ...
    parser.add_argument('--skip-single-layers', action='store_true', default=True,
                       help="Skip single-layer geometric calibration experiments")
    parser.add_argument('--force-recompute', action='store_true',
                       help="Pass --force-recompute to compare_dac_geometric.py and do not skip existing comparison JSONs")
    ...
    parser.add_argument('--baselines-only', action='store_true',
                       help="Run only standard baseline calibrators in each job, preserving existing non-baseline JSON sections")
    parser.add_argument('--compute-brier-top-label', action='store_true',
                       help="Compute/save top-label Brier as standard_baselines.<method>.top_label_brier")
    parser.add_argument('--methods', nargs='+', action='append', default=None,
    ...
    if args.baselines_only and args.methods:
        parser.error("--baselines-only cannot be combined with --methods")
    if args.baselines_only and (args.deep_ensemble_scan or args.eval_corruptions or args.mce_only):
        parser.error("--baselines-only cannot be combined with --deep-ensemble-scan, --eval-corruptions, or --mce-only")
    ...
    if args.deep_ensemble_scan:
        required = ['models', 'datasets', 'training_methods',
    ...
    experiments = filter_existing_experiments(
        ...
        compute_brier_top_label=args.compute_brier_top_label,
    )
            ...
            for training_method in sorted(experiments_by_seed[seed][model].keys()):
                ...
                for exp in sorted_exps:
                    ...
                    if config.get('deep_ensemble_scan', False):
                        ...
                        job_name = f"baseline_tlb_{exp['model_name']}_{exp['dataset_name']}_{exp['training_method']}_s{exp['seed']}"
                    else:
