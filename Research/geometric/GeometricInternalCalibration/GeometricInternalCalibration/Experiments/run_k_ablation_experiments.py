#!/usr/bin/env python3
"""
run_k_ablation_experiments.py

Parallel runner for the k-neighbor ablation experiment (Experiments/run_k_ablation.py)
using SLURM. Submits jobs for different (model, dataset, seed) combinations.

Usage:
    python run_k_ablation_experiments.py --config configs/k_ablation_sweep.yaml

Or with inline arguments:
    python run_k_ablation_experiments.py \
        --models resnet18 densenet121 \
        --datasets cifar10 cifar100 \
        --seeds 11 12 13 \
        --results-base-dir /path/to/models \
        --output-base-dir results/k_ablation
"""
...
def generate_sbatch_command(
    ...
    skip_ood: bool = False,
    layer_modes: List[str] = None,
    conda_env: str = "geo_cuda12",
    run_trust_score: bool = False,
) -> str:
    """
    Generate an sbatch command string for a single k-ablation job.
    ...
    log_dir = Path(output_dir) / "_k_ablation_logs"
    ...
    python_cmd = [
        sys.executable,
        str(project_root / "Experiments" / "run_k_ablation.py"),
    ...
    if layer_modes:
        python_cmd.extend(["--layer_modes"] + layer_modes)
    if skip_coordinate:
        python_cmd.append("--skip_coordinate")
    if skip_ood:
        python_cmd.append("--skip_ood")
    if run_trust_score:
        python_cmd.append("--run_trust_score")
    ...
    wrap_script = f"""
echo '========================================'
echo '🔬 SLURM JOB: K-Ablation Study'
echo '========================================'
...
echo '  k_values   : {" ".join(str(k) for k in k_values)}'
echo '  Layer Modes: {" ".join(layer_modes) if layer_modes else "default"}'
echo '  Skip Coord : {skip_coordinate}'
echo '  Skip OOD   : {skip_ood}'
echo '  TrustScore : {run_trust_score}'
echo '----------------------------------------'
...
def check_metrics_in_results(result_file: str) -> Tuple[bool, str]:
    """
    Check if a completed results file contains the summary section produced by run_k_ablation.py.

    Args:
        result_file: Path to the k-ablation results JSON file

    Returns:
        (is_complete, reason): True if summary exists, False with reason otherwise
    """
    import json
    ...
    try:
        ...
        return False, "Missing 'summary' key"
...
def filter_existing_experiments(
    ...
    Args:
        ...
        output_base_dir: Base output directory for k-ablation results
    ...
    for exp in experiments:
        result_file = os.path.join(
            output_base_dir,
            f"{exp['model_name']}_{exp['dataset_name']}_seed{exp['seed']}_k_ablation.json"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Parallel runner for k-neighbor ablation experiment",
    ...
    parser.add_argument('--training-methods', nargs='+', default=None,
                        ...
                        help="Training methods (e.g., cross_entropy focal)")
    parser.add_argument('--k-values', nargs='+', type=int, default=[1, 5, 10, 25, 50, 100, 200],
                        dest='k_values',
                        help="K values to evaluate in each job")
    parser.add_argument('--layer-modes', nargs='+', default=['random_6', 'dac_layers', 'last_layer'],
                        dest='layer_modes',
                        help="Layer selection modes to test")
    parser.add_argument('--skip-coordinate', action='store_true', dest='skip_coordinate',
                        help="Skip coordinate sampling experiments")
    parser.add_argument('--skip-ood', action='store_true', dest='skip_ood',
    ...
    parser.add_argument('--output-base-dir', type=str, default=None,
                        help="Output directory for k-ablation results")
    ...
    parser.add_argument('--run-trust-score', action='store_true',
                        dest='run_trust_score',
                        help="Also run FAISS-based Trust Score baseline in each k-ablation job")
    ...
    for seed in sorted(experiments_by_seed.keys()):
        ...
        for model in sorted(experiments_by_seed[seed].keys()):
            ...
            for exp in sorted_exps:
                job_name = f"kabl_{exp['model_name']}_{exp['dataset_name']}_{exp['training_method']}_s{exp['seed']}"
                ...
                try:
                    sbatch_cmd = generate_sbatch_command(
                        ...
                        layer_modes=args.layer_modes,
                        conda_env=args.conda_env,
                        run_trust_score=args.run_trust_score,
                    )
    ...
    if args.dry_run:
        ...
        logger.info(f"📁 SLURM logs in: {args.slurm_output_dir}/_k_ablation_logs/")
