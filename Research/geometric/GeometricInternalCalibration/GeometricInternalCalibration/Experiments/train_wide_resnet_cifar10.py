...
class DINOv2Classifier(nn.Module):
    ...
    def __init__(
        ...
        if timm_name is None:
            raise ValueError(f"Unknown DINOv2 model: {model_name}. Choose from {list(timm_name_map.keys())}")
...
def build_dataloaders(
    ...
    if ds_name == "cifar100":
        ...
        from Data.tiny_imagenet import get_train_valid_loader as tiny_imagenet_get_train_valid_loader
...
def run_training(args: argparse.Namespace) -> None:
    ...
    use_reduce_lr_on_plateau = isinstance(scheduler, optim.lr_scheduler.ReduceLROnPlateau)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_metrics = evaluate_model(
            model, val_loader, device, loss_fn, args.ece_bins
        )
        ...
        if use_reduce_lr_on_plateau:
            ...
            if new_lr < current_lr:
                ...
                if not lr_has_decayed:
                    lr_has_decayed = True
                    epochs_since_lr_decay = 0  # Only reset on first decay to start tracking
                    logger.info(f"First LR decay detected at epoch {epoch}. Starting early stopping counter.")
                else:
                    logger.info(
