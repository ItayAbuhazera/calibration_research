...
def wide_resnet_cifar(temp=1.0, num_classes=10, depth=26, width=10, **kwargs):
    """
    Build a Wide-ResNet model for CIFAR-style datasets.

    Supports both canonical (depth = 6n + 4) and legacy (depth = 6n + 2)
    formulations so that WRN-28-10 can be instantiated alongside older depth-26
    checkpoints without breaking compatibility.
    """
    if (depth - 4) % 6 == 0:
        n = (depth - 4) // 6
    elif (depth - 2) % 6 == 0:
        ...
        raise ValueError(
            f"Unsupported depth={depth}. Expected depth satisfying 6n+4 or 6n+2."
        )
