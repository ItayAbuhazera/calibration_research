...
- `geometric/GeometricInternalCalibration/GeometricInternalCalibration/` — RGC
  ...
  so, cite it but flag it as backup-sourced in the note.
...
index, never a source). Every claim you add must trace to one of the first
two, or be explicitly labeled interpretation/hypothesis. Don't silently
reconcile conflicting artifacts — record the conflict.

## After running or reading about an experiment, you may

- Add a dated `ResearchBrain/05_Experiments/` card citing exact artifact
  paths and seeds.
- Extend `02_Observations/` or `03_Failure_Modes/` notes, but only with a
  concrete evidence source and a "what this does NOT establish" section.
- Update the relevant `10_Projects/` artifact ledger.
- Append to (never rewrite) the current `08_Weekly_Synthesis/` note.

You may **not**, without the user explicitly asking: edit historical result
artifacts/checkpoints/logs in a project repo, overwrite a preregistration, or
rewrite past synthesis history instead of appending.
...
and mark the pattern as an open question, not a claim. If a task tells you
not to touch `06_Ideas/`, that means zero edits, including "just a link."
